...
const Router = (() => {
  ...
  // ── Chat-view deep link notice (#session/<id>?prompt=…) ─────────────────
  // The welcome view renders its own notice; this is the same contract for a link
  // that targets an existing session. That composer lives in sessions.js, so the
  // clearing side is driven from a document-level listener instead.
  let _chatDeepLinkNotice = null;   // { kind: "normal" | "long" | "tooLong", count, max }

  function _renderChatDeepLinkNotice() {
    const el = $("chat-deeplink-notice");
    if (!el) return;
    if (!_chatDeepLinkNotice) {
      el.textContent = "";
      el.style.display = "none";
      return;
    }
    const n = _chatDeepLinkNotice;
  ...
  function _applyChatDeepLinkNotice(deep) {
    if (deep && deep.tooLong) {
      _chatDeepLinkNotice = { kind: "tooLong", count: 0, max: deep.max };
    } else if (deep && deep.prompt) {
      _chatDeepLinkNotice = {
      ...
      _chatDeepLinkNotice = null;
    }
    _renderChatDeepLinkNotice();
  }

  // The notice survives until the box is cleared — the rule the welcome view
  // follows too. Watching in the capture phase keeps sessions.js untouched.
  document.addEventListener("input", (event) => {
    if (!_chatDeepLinkNotice || _chatDeepLinkNotice.kind === "tooLong") return;
    const input = event.target;
    if (!input || input.id !== "user-input") return;
    if (!Composer.hasContent(input)) {
      _chatDeepLinkNotice = null;
      _renderChatDeepLinkNotice();
    }
  }, true);
  ...
  // ── External deep links ─────────────────────────────────────────────────
  // "#new?prompt=<urlencoded>[&agent=<id>]" lets an outside page hand a task
  // over to a fresh session. The shape follows claude-cli:// and
  // claude://code/new: the text only lands in the composer for the user to
  // review and send, and the agent is preselected only when it exists.
  // The query lives in the hash, so it is never sent to the server — the
  // prompt cannot leak into a request log or any access record.
  const DEEP_LINK_MAX_PROMPT  = 5000;  // longer prompts are dropped, never truncated
  const DEEP_LINK_LONG_PROMPT = 1000;  // above this the notice carries a character count

  function _deepLinkParams(query) {
    const raw   = query.get("prompt") || query.get("q") || "";
    const agent = query.get("agent") || "";
    if (!raw && !agent) return null;
    const tooLong = raw.length > DEEP_LINK_MAX_PROMPT;
    return {
      prompt:  tooLong ? "" : raw,
      agent:   agent || null,
      tooLong,
      long:    !tooLong && raw.length > DEEP_LINK_LONG_PROMPT,
      max:     DEEP_LINK_MAX_PROMPT,
    };
  }
  ...
  function _parseHash(hash) {
    ...
    // Split the query off before matching paths: it must never end up inside a
    // path param ("#session/abc?prompt=x" used to resolve to the id "abc?prompt=x").
    const qAt  = h.indexOf("?");
    const path = qAt === -1 ? h : h.slice(0, qAt);
    const query = new URLSearchParams(qAt === -1 ? "" : h.slice(qAt + 1));
    if (path === "new") {
      const deepLink = _deepLinkParams(query);
      return { view: "welcome", params: deepLink ? { deepLink } : {} };
    }
    if (path === "tasks")             return { view: "tasks",    params: {} };
    if (path === "skills")            return { view: "skills",   params: {} };
    if (path === "channels")          return { view: "channels", params: {} };
    if (path === "mcp")               return { view: "mcp",      params: {} };
    if (path === "extensions")        return { view: "extensions", params: {} };
    const mExtDetail = path.match(/^extensions\/(.+)$/);
    if (mExtDetail) {
      const source = query.get("source") || null;
      return { view: "extensions", params: { detailId: mExtDetail[1], source } };
    }
    if (path === "trash")             return { view: "trash",    params: {} };
    ...
    if (path === "memories")          return { view: "profile",  params: {} };
    if (path === "billing")           return { view: "billing",  params: {} };
    if (path === "settings")          return { view: "settings", params: {} };
    const mExt = path.match(/^ext\/(.+)$/);
    if (mExt)                         return { view: "ext-workspace", params: { id: mExt[1] } };
    const m = path.match(/^session\/(.+)$/);
    if (m) {
      const deepLink = _deepLinkParams(query);
      return { view: "session", params: deepLink ? { id: m[1], deepLink } : { id: m[1] } };
    }
  ...
  async function _apply(view, params = {}) {
    ...
    switch (view) {
      ...
      case "session": {
        ...
        // #session/<id>?prompt=… seeds the draft before the switch, so _setActiveId
        // restores it through the normal draft path: prefilled, never sent.
        if (params.deepLink && params.deepLink.prompt) {
          Sessions.setDraft(id, params.deepLink.prompt);
        }
        Sessions._setActiveId(id);
        _applyChatDeepLinkNotice(params.deepLink);
      ...
      default:  // "welcome"
        ...
        if (window.NewSessionView && NewSessionView.onPanelShow) {
          NewSessionView.onPanelShow(params);
        }
        break;
    }
  ...
  return {
    ...
    /** Pure hash → { view, params } resolution, including deep-link parsing.
     *  Exported so the routing rules can be tested without a DOM. */
...
// ── Mobile soft-keyboard viewport ─────────────────────────────────────────
// iOS/Android keyboards resize only the visual viewport, so a 100dvh layout
// keeps the composer underneath the keyboard. Mirror the visual viewport into
// --app-viewport-height, consumed by the mobile body rule in app.css.
(() => {
  const vv = window.visualViewport;
  if (!vv) return;

  function syncViewportHeight() {
    // offsetTop is non-zero while the browser pans inside the visual viewport,
    // so add it back to keep the layout bottom on the visible bottom edge.
    document.documentElement.style.setProperty("--app-viewport-height", (vv.height + vv.offsetTop) + "px");
  }
