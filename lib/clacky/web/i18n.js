...
const I18n = (() => {
  ...
  const TRANSLATIONS = {
    en: {
      ...
      "settings.search.desc":              "By default web_search scrapes DuckDuckGo and Bing, which needs no key but returns lower-quality results. Select a search provider to use an API instead — it falls back to the built-in engines if the provider fails.",
    ...
    zh: {
      ...
      "settings.search.desc":              "默认 web_search 抓取 DuckDuckGo 和 Bing，无需密钥但结果质量较低。选择搜索服务商可改用 API 搜索，服务商失败时会自动回退到内置引擎。",
