...
const ModelPicker = (() => {
  ...
  async function _runBenchmark(container, btn, label, hint, onBenchmark) {
    ...
    container.querySelectorAll(".sib-model-option").forEach(opt => {
      ...
      const cell = opt.querySelector(".sib-model-latency");
      if (!id || !cell) return;
      _benchCache[id] = { pending: true };
      fillLatencyCell(cell, _benchCache[id]);
    });
    ...
    try {
      ...
      (results || []).forEach(r => {
        ...
        const cell = opt.querySelector(".sib-model-latency");
        if (!cell) return;
        fillLatencyCell(cell, _benchCache[r.model_id]);
      });
      ...
      container.querySelectorAll(".sib-model-option").forEach(opt => {
        ...
        if (id && _benchCache[id] && _benchCache[id].pending) {
          ...
          const cell = opt.querySelector(".sib-model-latency");
          if (!cell) return;
          fillLatencyCell(cell, undefined);
        }
      });
    } finally {
  ...
  // ── Main entry point ────────────────────────────────────────────────────
  // opts:
  //   models           [{id, model|display_model, type, remark, runtime_id, ...}]
  ...
  //   isSelectable(model) — optional live-session compatibility guard
  async function populate(container, opts) {
    const {
      models, currentId, onSelect,
      mediaCaps, subInfo, onSwitchSubModel,
      onBenchmark, onConfigureMedia, isSelectable,
    } = opts;
    ...
    const _nameCounts = models.reduce((acc, m) => {
      const name = RuntimeProvider.displayModel(m);
    ...
    models.forEach(m => {
      ...
      const selectable = typeof isSelectable !== "function" || isSelectable(m);
      if (!selectable) {
      ...
      const displayName = hasActiveOverride
        ? subInfo.current
        : RuntimeProvider.displayModel(m);
      ...
      const runtimeHint = m.runtime_id
        ? I18n.t("runtime.provider.dynamicHint")
        : "";
      if (remark || runtimeHint || _nameCounts[displayName] > 1) {
        left.classList.add("has-sub");
        const host = (() => {
          try { return new URL(m.base_url).host; } catch { return m.base_url || ""; }
        })();
        const subBits = [remark, runtimeHint, host, m.api_key_masked].filter(Boolean);
        if (subBits.length) {
          const subLine = document.createElement("span");
          subLine.className = "sib-model-name-sub";
          subLine.textContent = subBits.join(" · ");
          left.appendChild(subLine);
          opt.title = `${displayName} · ${subBits.join(" · ")}`;
        }
      }
      ...
      if (selectable) {
        ...
        opt.title = I18n.t("runtime.provider.newSessionHint");
      }
