...
const Notify = (() => {
  ...
  function _notifyDesktop(sid, awaiting) {
    ...
    const titleKey = awaiting ? "notify.desktop.awaiting.title" : "notify.desktop.title";
    const bodyKey  = awaiting ? "notify.desktop.awaiting.body"  : "notify.desktop.body";
    const title = typeof I18n !== "undefined"
      ? I18n.t(titleKey)
      : (awaiting ? "Waiting for your reply" : "Task complete");
    const body = typeof I18n !== "undefined"
      ? I18n.t(bodyKey)
      : (awaiting ? "The agent asked you a question. Click to answer." : "A background task finished. Click to open it.");
  ...
  function onTaskFinished(sid, awaiting) {
    if (!enabled()) return;
    if (!_userIsAway(sid)) return;
    _play();
    _notifyDesktop(sid, awaiting);
  }
