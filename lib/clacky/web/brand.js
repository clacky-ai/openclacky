...
const Brand = (() => {
  ...
  function _applyHeaderLogo() {
    fetch("/api/brand").then(r => r.json()).then(info => {
      ...
      // Wrap the brand area in a link if homepage_url is set
      if (info.homepage_url && brandWrap) {
        const existing = brandWrap.parentElement;
        if (existing && existing.tagName !== "A") {
          const link       = document.createElement("a");
          link.href        = info.homepage_url;
          link.target      = "_blank";
          link.rel         = "noopener noreferrer";
          link.style.textDecoration = "none";
          link.style.display        = "contents";
          brandWrap.parentNode.insertBefore(link, brandWrap);
          link.appendChild(brandWrap);
        } else if (existing && existing.tagName === "A") {
          existing.href = info.homepage_url;
        }
      }
