...
const Sessions = (() => {
  ...
  let   _totalCount        = null;    // server-reported total, independent of what's paged in
  let   _loadingMore       = false;
  // ── Bulk-delete selection ────────────────────────────────────────────────
  // Scoped to the sidebar list only: the search overlay renders its rows into
  // its own container, so `.selecting` (set on #sidebar-list) never reaches it.
  let   _selectMode        = false;
  const _selected          = new Set();  // session ids ticked in selection mode
  let   _deletingSelected  = false;      // guards against double-submit
  // Search state
  ...
  // Shared by the row delete item and the header list menu's bulk-delete item.
  ...
  function _initMessageHistory() {
    ...
    const selectBar = document.getElementById("session-select-bar");
    if (selectBar) {
      selectBar.querySelector("[data-select-cancel]")
        .addEventListener("click", () => Sessions.exitSelectMode());
      selectBar.querySelector("[data-select-delete]")
        .addEventListener("click", () => Sessions.deleteSelected());
    }

    const sessionsMenuBtn = document.getElementById("btn-sessions-menu");
    if (sessionsMenuBtn) {
      sessionsMenuBtn.addEventListener("click", e => {
        e.stopPropagation();
        Sessions._showListMenu(sessionsMenuBtn);
      });
    }

    document.addEventListener("keydown", (e) => {
      if (e.key === "Escape" && Sessions.selectMode) Sessions.exitSelectMode();
    });
  }
  ...
  // Toggles one row's tick. Mutates a single class instead of re-rendering so
  // the sidebar keeps its scroll position while the user picks rows.
  function _toggleSelect(id, el) {
    if (_selected.has(id)) {
      _selected.delete(id);
  ...
  function _renderSessionItem(container, s) {
    ...
    // Selection is re-applied on every render so pagination / WS patches never
    // lose ticks the user already made.
    if (_selected.has(s.id)) el.classList.add("selected");

    el.innerHTML = `
      <span class="session-select" aria-hidden="true"><span class="session-select-box"><svg viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M3 7.5l2.5 2.5L11 4.5" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg></span></span>
    ...
    el.onclick = (e) => {
      ...
      // Selection mode short-circuits *before* the dblclick timer so ticking a
      // row feels instant instead of lagging 200ms behind the click. Scoped to
      // the sidebar for the same reason as the CSS: rows rendered into the
      // search overlay have no visible checkbox, so they must stay openable.
      if (_selectMode && el.closest("#sidebar-list")) {
        _toggleSelect(s.id, el);
        return;
      }
  ...
  return {
    ...
    init() {
      ...
      document.addEventListener("langchange", () => {
        Sessions.renderList();
        // The bar's counter and delete label are written imperatively, so
        // renderList alone leaves them in the previous language.
        Sessions._renderSelectBar();
      });
    ...
    setAll(list, hasMore = false, groups = {}, total = null) {
      _sessions.length = 0;
      _sessions.push(...list);
      _hasMore = !!hasMore;
      if (total !== null && total !== undefined) _totalCount = Number(total);
    ...
    remove(id) {
      ...
      if (idx !== -1) {
        ...
        else if (_totalCount !== null) _totalCount = Math.max(0, _totalCount - 1);
      ...
      // A row deleted elsewhere (WS broadcast, project delete) must not linger
      // in the tick set, or the select bar would count a session that's gone.
      _selected.delete(id);
    },
    ...
    async loadMore() {
      ...
      try {
        ...
        if (data.total !== null && data.total !== undefined) _totalCount = Number(data.total);
    ...
    /**
     * List-level "···" menu in the sessions header. Bulk delete lives here so
     * no single row's menu can masquerade as its entry point. The item reuses
     * the row-menu danger styling verbatim, minus the top separator that only
     * makes sense above the row menu's own delete entry.
     */
    _showListMenu(anchor) {
      Sessions._closeActionsMenu();

      const menu = document.createElement("div");
      menu.className = "session-actions-menu";
      menu.innerHTML = `
        <div class="session-actions-menu-item session-actions-menu-item--danger session-actions-menu-item--danger-follow" data-action="selectMultiple">
          <span class="session-actions-menu-icon">${ICON_TRASH}</span>
          <span class="session-actions-menu-label">${escapeHtml(I18n.t("sessions.actions.selectMultiple"))}</span>
        </div>`;

      menu.querySelector('[data-action="selectMultiple"]')
      ...
      // The header sits deep in the sidebar, so "right: <button edge>" (the
      // projects-organize style) pushes the menu off-screen at the minimum
      // sidebar width. Anchor to the button's left edge and clamp on all sides.
    ...
    /** Enter selection mode from the sidebar header button (nothing pre-ticked). */
    enterSelectMode() {
      _selectMode = true;
      _selected.clear();

      const list = document.getElementById("sidebar-list");
      if (list) list.classList.add("selecting");
      // Only mark rows, never re-render: a full renderList() would reset the
      // sidebar scroll position right as the user starts picking rows.
      document.querySelectorAll("#sidebar-list .session-item[data-session-id]").forEach(el => {
        el.classList.remove("selected");
      });
      Sessions._renderSelectBar();
    },
    ...
    exitSelectMode() {
      ...
      document.querySelectorAll("#sidebar-list .session-item.selected").forEach(el => {
        el.classList.remove("selected");
      });
      Sessions._renderSelectBar();
    },

    get selectMode() { return _selectMode; },

    selectedIds() { return Array.from(_selected); },

    /** Sync the floating bar's counter / button states with `_selected`. */
    _renderSelectBar() {
      const bar = document.getElementById("session-select-bar");
      if (!bar) return;

      bar.hidden = !_selectMode;
      if (!_selectMode) return;

      const n = _selected.size;
      const countEl = bar.querySelector("[data-select-count]");
      const delBtn  = bar.querySelector("[data-select-delete]");

      if (countEl) {
        countEl.textContent = _totalCount === null
          ? I18n.t("sessions.select.count", { n: n })
          : I18n.t("sessions.select.countOf", { n: n, total: _totalCount });
      }
      if (delBtn) {
        delBtn.disabled = n === 0 || _deletingSelected;
        const label = delBtn.querySelector("[data-select-delete-label]");
        if (label) label.textContent = I18n.t("sessions.select.delete", { n: n });
      }
    },

    /** Delete every ticked session, then leave selection mode. */
    async deleteSelected() {
      ...
      const confirmed = await Modal.confirm(I18n.t("sessions.select.confirmDelete", { n: ids.length }));
      if (!confirmed) return;
      ...
      const failed = [];
      let hitActive = false;
      // Batched instead of all-at-once: deleting a live session tears down its
      // agent thread server-side, so a 30-row pick shouldn't fire 30 at once.
      const CONCURRENCY = 4;
      try {
        for (let i = 0; i < ids.length; i += CONCURRENCY) {
          ...
          await Promise.all(batch.map(async id => {
            try {
              ...
              // 404 means it's already gone — converge to the server's view.
              if (res.ok || res.status === 404) {
                if (id === Sessions.activeId) hitActive = true;
                Sessions.remove(id);
              } else {
                failed.push(id);
              }
        ...
        // One repaint for the whole batch, not one per session.
        Sessions.renderList();
        Projects.renderSection();
        if (hitActive) Router.navigate("welcome");
        if (failed.length > 0) {
          Modal.toast(I18n.t("sessions.select.deleteFailed", { n: failed.length }), "error");
        }
      }
    },
