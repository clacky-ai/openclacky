  ...
  return {
    ...
    async loadMore() {
      ...
      try {
        ...
        const params = new URLSearchParams({ limit: "10", exclude_type: "cron" });
    ...
    async loadMoreCron({ reset = false } = {}) {
      ...
      try {
        const params = new URLSearchParams({ limit: "10", type: "cron" });
...
(function() {
  ...
  function showDirectoryPicker(currentDir, sessionId) {
    return new Promise((resolve) => {
      ...
      let pickerContext = null; // OS-specific labels; real paths remain unchanged
      ...
      async function fetchDirs(relPath, absolute = false) {
        if (sessionLess) {
          ...
          pickerContext = data.picker || pickerContext;
      ...
      function locationText(key, fallback, vars) {
        const value = I18n.t(key, vars || {});
        return value && value !== key ? value : fallback;
      }

      function commonFolderName(name) {
        const labels = {
          Desktop: ["sib.dir.location.desktop", "Desktop"],
          Documents: [
            pickerContext && pickerContext.os === "macos"
              ? "sib.dir.location.documentsMac"
              : "sib.dir.location.documents",
            "Documents"
          ],
          Downloads: ["sib.dir.location.downloads", "Downloads"],
          Pictures: ["sib.dir.location.pictures", "Pictures"],
          Movies: ["sib.dir.location.movies", "Movies"],
          Videos: ["sib.dir.location.videos", "Videos"],
          Music: ["sib.dir.location.music", "Music"],
          Public: ["sib.dir.location.public", "Public"],
          Applications: ["sib.dir.location.applications", "Applications"],
          Library: ["sib.dir.location.library", "Library"],
          Contacts: ["sib.dir.location.contacts", "Contacts"],
          Favorites: ["sib.dir.location.favorites", "Favorites"],
          Links: ["sib.dir.location.links", "Links"],
          "Saved Games": ["sib.dir.location.savedGames", "Saved Games"],
          Searches: ["sib.dir.location.searches", "Searches"]
        };
        const label = labels[name];
        return label ? locationText(label[0], label[1]) : null;
      }

      // Localize well-known OS locations without changing their real paths.
      function friendlyDirectoryName(absPath, fallback) {
        const context = pickerContext || {};
        const path = String(absPath || "").replace(/\/+$/, "") || "/";
        const pickerHome = String(context.picker_home || "").replace(/\/+$/, "");
        const desktop = String(context.desktop || "").replace(/\/+$/, "");

        if (context.os === "wsl") {
          if (path === "/") return locationText("sib.dir.location.thisComputer", "This computer");
          if (path === "/mnt") return locationText("sib.dir.location.windowsDisks", "Windows disks");
          const drive = path.match(/^\/mnt\/([a-z])$/i);
          if (drive) {
            const letter = drive[1].toUpperCase();
            return locationText("sib.dir.location.drive", `${letter} Drive`, { drive: letter });
          }
          if (/^\/mnt\/[a-z]\/Users$/i.test(path)) {
            return locationText("sib.dir.location.users", "Users");
          }
          if (pickerHome && path === pickerHome) {
            return locationText("sib.dir.location.home", "Home");
          }
          if (desktop && path === desktop) {
            return locationText("sib.dir.location.desktop", "Desktop");
          }
          if (pickerHome && path.startsWith(`${pickerHome}/`)) {
            const relative = path.slice(pickerHome.length + 1);
            if (!relative.includes("/")) return commonFolderName(relative) || fallback;
          }
        }

        if (context.os === "macos") {
          if (path === "/") return locationText("sib.dir.location.thisMac", "This Mac");
          if (path === "/Users") return locationText("sib.dir.location.users", "Users");
          if (path === "/Volumes") {
            return locationText("sib.dir.location.volumes", "Disks and external volumes");
          }
          if (path === "/Applications") {
            return locationText("sib.dir.location.applications", "Applications");
          }
          if (pickerHome && path === pickerHome) {
            return locationText("sib.dir.location.home", "Home");
          }
      ...
      function refreshBreadcrumb(absPath) {
        ...
        parts.forEach((part, i) => {
          if (i === 0 && part === "") {
            ...
            seg.textContent = friendlyDirectoryName("/", "/");
      ...
      async function navigateTo(dirPath, absolute = true, pushHistory = true) {
        ...
        try {
          ...
          refreshBreadcrumb(selectedPath || dirPath);
