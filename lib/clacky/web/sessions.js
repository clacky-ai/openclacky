...
const Sessions = (() => {
    ...
    el.innerHTML = `
      <div class="session-body">
        <div class="session-name">
          ${dotHtml}
          <span class="session-name__text">${GROUP_ICONS[source]} ${I18n.t(`sessions.group.${source}`)} (${count})</span>
        </div>
  ...
  function _updateChatHeader(groupSource) {
    ...
    if (groupSource) {
      ...
      if (groupHeader) {
        ...
        if (title) title.textContent = `${GROUP_ICONS[groupSource]} ${I18n.t(`sessions.group.${groupSource}`)}`;
      }
    } else {
  ...
  return {
    ...
    _showActionsMenu(button, session) {
      ...
      const iconRemoveFromProject = `<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"/><path d="M8 13h8"/><path d="m13 10 3 3-3 3"/></svg>`;
