...
const Sessions = (() => {
  ...
  return {
    ...
    _setActiveId(id) {
      ...
      // Hydrate todos from the session row so the panel survives a full page
      // reload (WS todo_update only fires when the agent changes todos; without
      // this, a freshly reloaded page shows an empty panel until the next
      // todo_manager call).
      if (id && !Sessions._sessionTodos[id]) {
        Sessions.findOrFetch(id).then(s => {
          if (!s) return;
          if (!Sessions._sessionTodos[id]) {
    ...
    _renderTodoPanel() {
      ...
      // No active session or no todos → hide.
      ...
      const items = todos.map(t => {
        ...
        const idTag = (t && t.id != null) ? `<span class="todo-id">#${t.id}</span>` : "";
        return `<div class="${cls}"><span class="todo-icon">${icon}</span>${idTag}<span class="todo-text">${text}</span></div>`;
      }).join("");

      const collapsedCls = Sessions._todoCollapsed ? " collapsed" : "";
      const toggle = Sessions._todoCollapsed ? "▶" : "▼";
      panel.innerHTML =
        `<div class="todo-header"><span class="todo-toggle">${toggle}</span><span>${title}</span><span class="todo-count">${completed}/${total}</span></div>` +
