  ...
  return {
    ...
    _renderTodoPanel() {
      ...
      const items = todos.map(t => {
        ...
        const icon = "";  // circle/check drawn by CSS inside .todo-icon
        const cls = done ? "todo-item done" : "todo-item";
        const text = escapeHtml(String((t && t.task) || ""));
        return `<div class="${cls}"><span class="todo-icon">${icon}</span><span class="todo-text">${text}</span></div>`;
      }).join("");

      const collapsedCls = Sessions._todoCollapsed ? " collapsed" : "";
      panel.innerHTML =
        `<div class="todo-header"><span class="todo-toggle"></span><span>${title}</span><span class="todo-count">${completed}/${total}</span></div>` +
