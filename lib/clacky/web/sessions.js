...
const Sessions = (() => {
  ...
  return {
    ...
    /** Enter selection mode, pre-ticking the row the menu was opened from. */
    enterSelectMode(seedId) {
      _selectMode = true;
      _selected.clear();
      if (seedId) _selected.add(seedId);
      ...
      document.querySelectorAll("#sidebar-list .session-item[data-session-id]").forEach(el => {
        el.classList.toggle("selected", _selected.has(el.dataset.sessionId));
      });
      Sessions._renderSelectBar();
    },
    ...
    _showActionsMenu(button, session) {
      ...
      menu.addEventListener("click", async (e) => {
        ...
        if (action === "fork") {
          ...
          Sessions.enterSelectMode(session.id);
        }
      });
