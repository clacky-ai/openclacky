  ...
  return {
    ...
    add(session) {
      if (!_sessions.find(s => s.id === session.id)) {
        ...
        if (session.source === "cron" && !session.project_id) _cronCount++;
      }
    },
    ...
    patch(id, fields) {
      ...
      const wasCronVisible = before?.source === "cron" && !before?.project_id;
      ...
      const isCronVisible = updated?.source === "cron" && !updated?.project_id;
      if (wasCronVisible && !isCronVisible) _cronCount = Math.max(0, _cronCount - 1);
      else if (!wasCronVisible && isCronVisible) _cronCount++;
    ...
    remove(id) {
      ...
      if (idx !== -1) {
        if (_sessions[idx].source === "cron" && !_sessions[idx].project_id) _cronCount = Math.max(0, _cronCount - 1);
    ...
    renderList({ scrollToActive = false } = {}) {
      ...
      if (isCronView) {
        ...
        const cronHasRunning = _sessions.some(s => s.source === "cron" && !s.project_id && s.status === "running");
        ...
        // Only append at the bottom when all pages are exhausted; otherwise
        // the entry would show prematurely on the first page even though
        // its time position is several pages deeper.
        if (!cronEntryRendered && !_hasMore) {
          _renderCronGroupItem(list, _cronCount, cronHasRunning);
        }
      } else {
