...
const Sessions = (() => {
  ...
  let   _activeId          = null;
  let   _activeTab         = "manual";  // "manual" | "cron" | "channel"
  // Per-source pagination state (manual/cron/channel for general tabs; coding for coding section)
  const _hasMoreBySource   = { manual: false, cron: false, channel: false, coding: false };
  const _loadingMoreSource = { manual: false, cron: false, channel: false, coding: false };
  ...
  return {
    ...
    /** Load older sessions for a specific source tab.
     *  source: "manual" | "cron" | "channel" | "coding"
     */
    async loadMoreSessions(source) {
      ...
      try {
        // Cursor: oldest created_at among sessions of this source currently loaded
        const sourceSessions = source === "coding"
          ? _sessions.filter(s => s.agent_profile === "coding")
          : _sessions.filter(s => (s.source || "manual") === source && s.agent_profile !== "coding");

        const oldest = sourceSessions.reduce((min, s) => {
          if (!s.created_at) return min;
          return (!min || s.created_at < min) ? s.created_at : min;
        }, null);

        let url = `/api/sessions?limit=10&source=${encodeURIComponent(source)}`;
    ...
    renderList() {
      ...
      // Classify sessions
      const isCoding  = s => s.agent_profile === "coding";
      const isGeneral = s => !isCoding(s);

      const generalSessions = _sessions.filter(isGeneral).sort(byTime);
      const codingSessions  = _sessions.filter(isCoding).sort(byTime);

      // ── Tab visibility: show tabs only if cron or channel sessions exist ──
      const hasNonManual = generalSessions.some(s => s.source && s.source !== "manual");
      const tabBar = $("session-tabs");
      if (tabBar) tabBar.style.display = hasNonManual ? "" : "none";

      // If tabs are hidden, always show manual sessions regardless of _activeTab
      const effectiveTab = hasNonManual ? _activeTab : "manual";
