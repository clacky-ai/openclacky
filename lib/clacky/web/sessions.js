...
const Sessions = (() => {
  ...
  return {
    ...
    /** Pending composer attachments (uploaded but not yet sent), both file
     *  uploads and staged images. Exposed so FileAC can offer them in the
     *  @-mention dropdown — /api/files only lists the session working_dir,
     *  which never contains freshly uploaded files. Images carry no server
     *  path (they travel as data URLs), so `path` is null for them. */
    pendingAttachments() {
      return [
        ..._pendingFiles.map(f => ({ name: f.name, path: f.path })),
        ..._pendingImages.map(img => ({ name: img.name, path: null })),
      ];
    },
...
// ── FileAC — @ file mention autocomplete ──────────────────────────────────
//
// Handles the "@xxx" file mention autocomplete UI above the message input.
// When user types @, shows a dropdown with files from the session's working directory.
// Selected files are inserted as @filename mentions in the input.
//
// Depends on: Sessions (activeId), global $ helper.
// ─────────────────────────────────────────────────────────────────────────
const FileAC = (() => {
  let _initialized = false;
  let _visible = false;
  let _activeIndex = -1;
  let _items = [];  // [{ path, kind }]
  let _query = "";
  let _fetchSeq = 0;
  let _fetchTimer = null;

  const _dropdownEl = () => $("file-autocomplete");
  const _listEl = () => $("file-autocomplete-list");
  const _inputEl = () => $("user-input");

  /** Initialize FileAC — called once when Sessions.init() runs. */
  function init() {
    ...
    // Some browsers (notably older Safari) fire no non-composing `input`
    // after an IME commit, so the final CJK text would never re-run the
    // @ trigger. Re-check on compositionend; the 150 ms debounce absorbs
    // the duplicate run on browsers that do fire `input`.
    input.addEventListener("compositionend", () => _onInput({ isComposing: false }));
    input.addEventListener("input", _syncHighlight);
    input.addEventListener("scroll", _syncHighlight);

    // Close dropdown when clicking outside
    document.addEventListener("click", (e) => {
      if (_visible && !e.target.closest("#file-autocomplete") && e.target !== input) {
        hide();
      }
    });
  }

  /** Render @file mentions as styled tags in the highlight layer behind the
   *  (transparent-text) textarea. Keeps the composer looking like the design:
   *  chosen files appear as tags, not plain text. */
  function _syncHighlight() {
    const input = _inputEl();
    const hl = document.getElementById("user-input-highlight");
    if (!input || !hl) return;
    hl.innerHTML = _renderHighlightHtml(input.value);
    hl.style.transform = input.scrollTop ? `translateY(${-input.scrollTop}px)` : "";
  }

  /** Escape a text node body for innerHTML (no quote escaping — quotes must
   *  survive so the @"file with spaces" regex still matches). */
  function _escBody(s) {
    return String(s).replace(/[&<>]/g, c => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;" }[c]));
  }

  // Boundary rule shared by every mention regex in this module: an @ starts a
  // mention only at the beginning of the text or after a character that
  // CANNOT end an email local part. Without it, "foo@bar.com" (or the
  // look-alike "x@README.md") gets colored/treated as a mention. The class
  // must stay in sync with resolve_mention_attachments in http_server.rb,
  // which enforces the same rule server-side.

  function _renderHighlightHtml(text) {
    const safe = _escBody(text);
    // Wrap the FULL matched text (quotes included): the highlight layer must
    // render exactly the same characters as the textarea, or the visible text
    // drifts away from the real caret/selection positions. The boundary char
    // before @ is captured and re-emitted untouched.
    return safe.replace(/(^|[^A-Za-z0-9._%+-])(@"(?:[^"\\]|\\.)*"?|@[^\s@]+)/g,
      (_m, boundary, mention) => `${boundary}<span class="file-mention-tag">${mention}</span>`);
  }

  /** Handle input events — detect @ trigger and show dropdown. */
  function _onInput(e) {
    // Ignore IME composition events (CJK pinyin, etc.) — the composer value
    // seen mid-composition is not what the user is typing, and the @ trigger
    // must not hijack the composition session.
    ...
    // Find the @ symbol before cursor. Supports both bare paths (@docs/x) and
    // quoted paths (@\"dir with space/…) where the closing quote may be
    // omitted while the user is still typing inside the directory.
    const textBeforeCursor = value.slice(0, cursorPos);
    const atMatch = textBeforeCursor.match(/(^|[^A-Za-z0-9._%+-])@(?:"((?:[^"\\]|\\.)*)"?|([^\s@]*))$/);

    if (atMatch) {
      _query = atMatch[2] !== undefined ? atMatch[2] : atMatch[3];
      _showDropdown(_query);
    } else {
      hide();
    }
  }

  /** Mention chip ranges in `value`: [start, textEnd, chipEnd].
   *  chipEnd absorbs the single trailing space inserted after a confirmed
   *  mention, so the space is treated as part of the chip when deleting. */
  function _mentionRanges(value) {
    const ranges = [];
    const re = /(^|[^A-Za-z0-9._%+-])@(?:"(?:[^"\\]|\\.)*"?|[^\s@]+)/g;
    let m;
    while ((m = re.exec(value)) !== null) {
      const start = m.index + m[1].length; // the @ itself; boundary char excluded
      const textEnd = m.index + m[0].length;
      let chipEnd = textEnd;
      if (value[chipEnd] === " ") chipEnd += 1;
      ranges.push([start, textEnd, chipEnd]);
    }
    return ranges;
  }

  /** Email-recipient style atomic mention editing.
   *  - Backspace with the caret right after a chip deletes the whole chip.
   *  - Delete with the caret right before a chip deletes the whole chip.
   *  - A partial selection overlapping a chip snaps to the chip bounds, so
   *    typing/deleting replaces the entire mention, never leaving fragments.
   *  The currently-typed @query (dropdown open) is NOT a confirmed chip and
   *  is excluded, so normal query editing keeps character-wise behavior. */
  function _atomicEdit(e, input) {
    const value = input.value;
    const a = input.selectionStart;
    const b = input.selectionEnd;

    // Active @query range (being typed, dropdown open) — excluded from atomicity.
    let activeStart = -1;
    if (_visible) {
      const am = value.slice(0, a).match(/(^|[^A-Za-z0-9._%+-])@(?:"((?:[^"\\]|\\.)*)"?|([^\s@]*))$/);
      if (am) activeStart = am.index + am[1].length;
    }

    const ranges = _mentionRanges(value);

    if (a === b && (e.key === "Backspace" || e.key === "Delete")) {
      for (const [s, textEnd, chipEnd] of ranges) {
        // Chip is atomic: caret anywhere inside it (or at its deleting edge)
        // deletes the whole mention, like an email recipient chip.
        const atChipEnd = e.key === "Backspace" && a > s && a <= chipEnd;
        const atChipStart = e.key === "Delete" && a >= s && a < chipEnd;
        if (!atChipEnd && !atChipStart) continue;
        if (s === activeStart) continue; // still typing this query
        e.preventDefault();
        e.stopImmediatePropagation();
        // Delete through execCommand so the native undo stack survives
        // (Ctrl+Z restores the chip). Manual edit as fallback.
        input.setSelectionRange(s, chipEnd);
        let done = false;
        try { done = document.execCommand("delete"); } catch (_) { /* noop */ }
        if (!done) {
          input.value = value.slice(0, s) + value.slice(chipEnd);
    ...
    const modifies = e.key === "Backspace" || e.key === "Delete" ||
      (e.key.length === 1 && !e.ctrlKey && !e.metaKey && !e.altKey);
    if (a !== b && modifies) {
      let na = a, nb = b;
      for (const [s, , chipEnd] of ranges) {
        if (s === activeStart) continue;
        if (na > s && na < chipEnd) na = s;
        if (nb > s && nb < chipEnd) nb = chipEnd;
      }
      if (na !== a || nb !== b) input.setSelectionRange(na, nb);
    }
  }

  /** Handle keydown events — navigate dropdown and select items. */
  function _onKeydown(e) {
    // IME composition: Backspace/arrows during CJK composition operate on
    // the composition window, not the composer value — never interpret them
    // as mention-chip edits.
    if (e.isComposing) return;

    const input = _inputEl();
    if (input) _atomicEdit(e, input);
    if (!_visible) return;
    ...
    switch (e.key) {
      case "ArrowDown":
        e.preventDefault();
        e.stopImmediatePropagation();
        _setActive(Math.min(_activeIndex + 1, _items.length - 1), true);
        break;
      case "ArrowUp":
        e.preventDefault();
        e.stopImmediatePropagation();
        _setActive(Math.max(_activeIndex - 1, 0), true);
        break;
      case "Enter":
        if (_activeIndex >= 0 && _activeIndex < _items.length) {
          e.preventDefault();
          e.stopImmediatePropagation();
          _selectItem(_items[_activeIndex]);
        } else {
          // No item selected — let Enter fall through to sendMessage
          hide();
        }
        break;
      case "Tab":
        if (_activeIndex >= 0 && _activeIndex < _items.length) {
          ...
          // Tab with nothing highlighted — select the first item
          e.preventDefault();
          e.stopImmediatePropagation();
          _selectItem(_items[0]);
        }
        // No items — let Tab fall through to default focus behavior
        break;
      case "Escape":
        e.preventDefault();
        e.stopImmediatePropagation();
  ...
  /** Update the highlighted item. Lightweight class toggle only — never
   *  rebuilds the DOM (rebuilding on mouseenter was the root cause of dead
   *  clicks and frozen keyboard navigation). `scroll` keeps the active item
   *  visible inside the dropdown when navigating with the keyboard. */
  function _setActive(index, scroll) {
    _activeIndex = index;
    const list = _listEl();
    if (!list) return;
    const items = list.querySelectorAll(".file-ac-item");
  ...
  /** Show the dropdown and fetch files. */
  function _showDropdown(query) {
    const dropdown = _dropdownEl();
    if (!dropdown) return;
    ...
    // Debounced server fetch: every keystroke would otherwise trigger a
    // recursive working-dir scan on the server. Previous results stay
    // visible until fresh ones arrive; a newer query supersedes an older
    // pending fetch (out-of-order responses would show stale entries).
    const seq = ++_fetchSeq;
    clearTimeout(_fetchTimer);
    _fetchTimer = setTimeout(async () => {
      await _fetchFiles(query);
      if (seq !== _fetchSeq || !_visible) return;
      _renderItems();
    }, 150);
  }
  ...
  async function _fetchFiles(query) {
    ...
    try {
      ...
      console.error("[FileAC] fetch files failed:", e);
      _items = [];
    }

    // Merge the composer's pending attachments (uploaded in this session,
    // not yet sent). /api/files only lists the session working_dir, and
    // uploads live in the uploads dir — so without this merge a file the
    // user attached moments ago can never be @-mentioned. Attachment names
    // are flat (no "/"), so path-navigation queries never match them.
    const q = (query || "").toLowerCase();
    const attached = (typeof Sessions.pendingAttachments === "function"
      ? Sessions.pendingAttachments() : [])
      .filter(f => f && f.name && (!q || f.name.toLowerCase().includes(q)))
      .map(f => ({ path: f.path, name: f.name, kind: "attachment" }));
    if (attached.length) _items = attached.concat(_items);
  }

  /** Render the file items in the dropdown. */
  function _renderItems() {
    ...
    if (_items.length === 0) {
      const empty = document.createElement("div");
      empty.className = "file-ac-empty";
      empty.textContent = _query ? I18n.t("files.noMatch") : I18n.t("files.empty");
    ...
    _items.forEach((item, index) => {
      const el = document.createElement("div");
      el.className = "file-ac-item";
      if (index === _activeIndex) el.classList.add("active");

      // Icon
      const icon = document.createElement("div");
      icon.className = "file-ac-icon";
      if (item.kind === "directory") {
        icon.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"></path></svg>';
      } else if (item.kind === "attachment") {
        // Paperclip — matches the composer's attachment chips, signalling
        // "a file you just attached in this conversation".
        icon.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21.44 11.05l-9.19 9.19a6 6 0 0 1-8.49-8.49l9.19-9.19a4 4 0 0 1 5.66 5.66l-9.2 9.19a2 2 0 0 1-2.83-2.83l8.49-8.48"></path></svg>';
      } else {
      ...
      // Name — attachments show their friendly upload name; working-dir
      // entries show their relative path.
      const name = document.createElement("div");
      name.className = "file-ac-name";
      name.textContent = item.name || item.path;

      el.appendChild(icon);
      el.appendChild(name);

      el.addEventListener("click", () => _selectItem(item));
      el.addEventListener("mouseenter", () => {
        // Lightweight highlight switch — MUST NOT rebuild the list here.
        // Rebuilding on hover destroyed click targets and fought the keyboard.
        _setActive(index, false);
      });
  ...
  /** Select a file item and insert @mention into input. */
  function _selectItem(item) {
    ...
    // Find the @ symbol and replace it with @filename
    const textBeforeCursor = value.slice(0, cursorPos);
    const atMatch = textBeforeCursor.match(/(^|[^A-Za-z0-9._%+-])@(?:"((?:[^"\\]|\\.)*)"?|([^\s@]*))$/);
    ...
    // Keep the boundary char (start-of-text / space / CJK) in place — only
    // the @query itself is replaced by the confirmed mention.
    const beforeAt = textBeforeCursor.slice(0, atMatch.index + atMatch[1].length);
    const afterCursor = value.slice(cursorPos);

    // Insert @path (with quotes if the path contains spaces). Attachments
    // are referenced by their friendly upload name — the server resolves the
    // name against the message's own attached files (see
    // resolve_mention_attachments), so no absolute upload path leaks into
    // the message text.
    const ref = item.kind === "attachment" ? (item.name || item.path) : item.path;
    let mention;
    if (/[\s"\\]/.test(ref)) {
      // Quote paths containing spaces/quotes/backslashes, escaping the last
      // two so the server's unescape (gsub /\\(.)/ -> \1) round-trips the
      // exact name — same rule the highlight-layer regexes already parse.
      mention = `@"${ref.replace(/["\\]/g, c => `\\${c}`)}"`;
    } else {
      mention = `@${ref}`;
    }

    // Directories: keep the dropdown open and browse the children, so the
    // user can drill into the folder instead of "selecting" a folder as a
    // mention. Files: finish the mention with a trailing space.
    //
    // IMPORTANT: route the change through a real `input` event instead of
    // calling _syncHighlight/_showDropdown/hide manually. The composer's
    // input pipeline also auto-grows the textarea (skills.js); skipping the
    // event left the box at one-line height after a mention wrapped, hiding
    // the wrapped line until the next keystroke.
    if (item.kind !== "directory") mention += " ";
  ...
  return {
    init,
    hide,
    /** Re-render the mention highlight layer (call after programmatic
     *  changes to the input value, e.g. clearing after send). */
    sync: _syncHighlight,
  };
})();

Clacky.FileAC = FileAC;
