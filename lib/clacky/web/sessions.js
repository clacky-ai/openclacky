...
const Sessions = (() => {
  ...
  return {
    ...
    updateTodos(todos) {
      const sid = _activeId;
      if (!sid) return;
      Sessions._sessionTodos[sid] = Array.isArray(todos) ? todos : [];
      Sessions._renderTodoPanel();
    },
    ...
    _renderTodoPanel() {
      ...
      panel.classList.toggle("collapsed", Sessions._todoCollapsed);
      panel.innerHTML =
        `<div class="todo-header">` +
        `<span class="todo-toggle">▾</span>📋 ${escapeHtml(title)} ` +
        `<span class="todo-count">${completed}/${total}</span></div>` +
        `<div class="todo-list">${items}</div>`;
      panel.style.display = "block";

      // Click header to toggle collapse (persisted via localStorage).
