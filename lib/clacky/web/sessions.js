...
const Sessions = (() => {
  ...
  // (running) dot only when any folded session in the group is currently
  // running, and render no dot at all when the group is idle.
  function _renderGroupItem(container, source, count, hasRunning = false) {
    const el = document.createElement("div");
    ...
    renderList({ scrollToActive = false } = {}) {
      ...
      if (_groupView) {
        ...
        const renderGroup = source => {
          const hasRunning = _sessions.some(s => s.source === source && !s.project_id && s.status === "running");
          _renderGroupItem(list, source, _groups.get(source).count, hasRunning);
        };
...
const FileAC = (() => {
  ...
  function _renderHighlightHtml(text) {
    ...
    // drifts away from the real caret/selection positions.
    return safe.replace(/@(?:"((?:[^"\\]|\\.)*)"?|[^\s@]+)/g,
      (match) => `<span class="file-mention-tag">${match}</span>`);
  }
  ...
  function _onInput(e) {
    ...
    if (atMatch) {
      _query = atMatch[1] !== undefined ? atMatch[1] : atMatch[2];
  ...
  function _mentionRanges(value) {
    ...
    while ((m = re.exec(value)) !== null) {
      ...
      ranges.push([m.index, textEnd, chipEnd]);
    }
    return ranges;
  }
  ...
  function _atomicEdit(e, input) {
    ...
    let activeStart = -1;
    if (_visible) {
      const am = value.slice(0, a).match(/@(?:"((?:[^"\\]|\\.)*)"?|([^\s@]*))$/);
      if (am) activeStart = am.index;
    }
