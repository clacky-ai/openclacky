...
const ExtensionsStore = (() => {
  ...
  let _onlyInstalled   = false;     // global overlay: show only installed items across all tabs
  let _loading    = false;
  ...
  const Extensions = {
    ...
    async load() {
      ...
      if (_filterMarket) return Extensions.loadMarketExtensions();
      _loading = true;
      _error   = null;
    ...
    /** Load marketplace extensions (all, not filtered by installed). */
    ...
    async setEnabled(id, enabled) {
      ...
      try {
        ...
        const activeTab = _filterSystem ? "system" : _filterBrand ? "brand" : "market";
