...
const ExtensionsView = (() => {
  ...
  let _renderedCount = 0;   // cards already appended to the list DOM
  ...
  function _renderList(payload) {
    ...
    const reset = !payload || payload.reset !== false;

    if (st.loading && reset) { _renderLoading(); return; }

    const list = st.extensions;
    ...
    for (let i = _renderedCount; i < list.length; i++) {
      container.appendChild(_renderCard(list[i]));
    }
    _renderedCount = list.length;

    _renderLoadMore(container, st.hasMore, st.loadingMore);

    _applyWarning(ExtensionsStore.state.error);
  }

  function _renderLoadMore(container, hasMore, loadingMore) {
    const existing = container.querySelector(".extensions-load-more");
    if (existing) existing.remove();
    if (!hasMore) return;
    // Only the marketplace tab is paginated; the other tabs are full loads.
    if (!ExtensionsStore.state.filterMarket) return;

    const btn = document.createElement("button");
    btn.type = "button";
    btn.className = "extensions-load-more";
    btn.textContent = loadingMore ? I18n.t("extensions.loadingMore") : I18n.t("extensions.loadMore");
    btn.disabled = loadingMore;
    btn.addEventListener("click", () => Extensions.loadMoreMarketExtensions());
  ...
  function _renderCard(ext) {
    ...
    const installedHtml = (ext.installed && ext.layer !== "builtin")
    ...
    const verifiedHtml = (ext.verified && !ext.installed)
    ...
    const isSystem = ext.layer === "builtin";
    const slug = ext.slug || (ext.id != null ? String(ext.id) : (ext.name || ""));
    const toggleHtml = isSystem ? _renderCardToggle(slug, ext.disabled) : "";

    const card = document.createElement("div");
    card.className = isSystem ? "extension-card extension-card-system" : "extension-card extension-card-clickable";
    ...
    if (isSystem) {
      const toggle = card.querySelector(".ext-card-toggle");
      if (toggle) {
        toggle.addEventListener("click", (e) => {
          e.stopPropagation();
          const id      = toggle.dataset.extToggle;
          const enabled = toggle.dataset.extEnabled === "1";
  ...
  function _renderCardToggle(id, disabled) {
    const checked = !disabled;
    return `
      <button type="button" class="ext-card-toggle${checked ? " ext-card-toggle-on" : ""}"
        data-ext-toggle="${escapeHtml(id)}" data-ext-enabled="${checked ? "1" : "0"}"
        title="${escapeHtml(I18n.t(checked ? "extensions.action.disable" : "extensions.action.enable"))}">
        <span class="ext-card-toggle-thumb"></span>
      </button>`;
  }

  function _saveCurrentTab() {
    try {
      var s = ExtensionsStore.state;
      var tab = "market";
      if (s.filterSystem) tab = "system";
      else if (s.filterBrand) tab = "brand";
      else if (s.filterInstalled) tab = "installed";
      sessionStorage.setItem(EXT_TAB_KEY, tab);
    } catch (_) {}
  }

  // Returns a human-readable button label for a given install job state.
  ...
  async function _applyBrandTab() {
    try {
      ...
      const marketTab = $("tab-extensions-market");
      if (brandTab) brandTab.style.display = data.branded ? "" : "none";
      if (marketTab) marketTab.style.display = data.branded ? "none" : "";
    } catch (_e) {
  ...
  function _wireDom() {
    ...
    document.querySelectorAll(".extensions-filter-tab").forEach(btn => {
      btn.addEventListener("click", () => {
        ...
        const installedFilterEl = $("extensions-installed-filter");
        if (filter === "system") {
          if (sortEl) sortEl.style.display = "none";
          if (installedFilterEl) installedFilterEl.style.display = "none";
          Extensions.setFilterSystem();
        } else if (filter === "brand") {
          if (sortEl) sortEl.style.display = "";
          if (installedFilterEl) installedFilterEl.style.display = "";
          Extensions.setFilterBrand(true);
        } else if (filter === "market") {
          if (sortEl) sortEl.style.display = "";
          if (installedFilterEl) installedFilterEl.style.display = "";
          Extensions.setFilterMarket();
    ...
    if (window.Skills && Skills.on) {
      Skills.on("brandStatus:changed", (p) => {
        ...
        if (marketTab) marketTab.style.display = p.branded ? "none" : "";
      });
    }
  ...
  const viewApi = {
    async onPanelShow(opts) {
      ...
      if (detailId) {
        if (sortEl) sortEl.style.display = "";
        Extensions.loadMarketExtensions();
        Extensions.loadDetail(detailId, opts && opts.source);
      } else {
        Extensions.closeDetail();

        let restoredTab = null;
        try { restoredTab = sessionStorage.getItem(EXT_TAB_KEY); } catch (_) {}
        if (restoredTab) {
          try { sessionStorage.removeItem(EXT_TAB_KEY); } catch (_) {}
        }

        let tab = restoredTab || "market";
        const tabBtn = document.querySelector(`.extensions-filter-tab[data-filter="${tab}"]`);
        if (tabBtn && tabBtn.style.display === "none") {
          const visible = Array.from(document.querySelectorAll(".extensions-filter-tab"))
            .find(b => b.style.display !== "none");
          tab = visible ? visible.dataset.filter : "market";
        }
