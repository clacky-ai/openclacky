...
const ExtensionsView = (() => {
  ...
  function _wireDom() {
    ...
    const newBtn = $("extensions-new-btn");
    if (newBtn) {
      newBtn.addEventListener("click", () => {
        const seed = I18n.t("extensions.newExt.seed");
        fetch("/api/ext/ext-studio/develop", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ idea: seed || null }),
        })
          .then((r) => r.json())
          .then((data) => {
            if (data && data.session_id) {
              const router = window.Clacky && window.Clacky.Router;
              if (router) router.navigate("session", { id: data.session_id });
            }
          })
          .catch(() => {});
      });
    ...
    document.addEventListener("langchange", () => {
      ...
      if (newBtn) newBtn.textContent = I18n.t("extensions.newExt.btn");
      _renderList();
      _renderDetail();
    });
