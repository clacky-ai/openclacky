...
const ChannelsView = (() => {
  ...
  function _render() {
    ...
    container.appendChild(_renderMessagesSettings());
  ...
  function _renderMessagesSettings() {
    const card = document.createElement("div");
    card.className = "channel-settings-card";
    card.appendChild(_renderGlobalSettings());
    const divider = document.createElement("div");
    divider.className = "channel-settings-divider";
    card.appendChild(divider);
    card.appendChild(_renderProcessMessagesSetting());
    return card;
  }

  function _renderGlobalSettings() {
    const on = ChannelsStore.state.statusMessages;
    const aria = I18n.t(on ? "channels.statusMessages.on" : "channels.statusMessages.off");
    const row = document.createElement("div");
    row.className = "channel-settings-row";
    row.innerHTML = `
      <div class="channel-status-messages-text">
        <span class="channel-status-messages-label">${I18n.t("channels.statusMessages.label")}</span>
        <span class="channel-status-messages-desc">${I18n.t("channels.statusMessages.desc")}</span>
      </div>
      <label class="toggle-switch" title="${_esc(aria)}">
        <input type="checkbox" id="status-msg-toggle" ${on ? "checked" : ""} aria-label="${_esc(aria)}">
        <span class="toggle-slider"></span>
      </label>
    `;
    row.querySelector("#status-msg-toggle")?.addEventListener("change", (ev) => _onStatusMessagesToggle(ev.target));
    return row;
  }

  function _renderProcessMessagesSetting() {
    const on = ChannelsStore.state.processMessages;
    const aria = I18n.t(on ? "channels.processMessages.on" : "channels.processMessages.off");
    const row = document.createElement("div");
    row.className = "channel-settings-row";
    row.innerHTML = `
      <div class="channel-status-messages-text">
        <span class="channel-status-messages-label">${I18n.t("channels.processMessages.label")}</span>
        <span class="channel-status-messages-desc">${I18n.t("channels.processMessages.desc")}</span>
      </div>
      <label class="toggle-switch" title="${_esc(aria)}">
        <input type="checkbox" id="process-msg-toggle" ${on ? "checked" : ""} aria-label="${_esc(aria)}">
        <span class="toggle-slider"></span>
      </label>
    `;
    row.querySelector("#process-msg-toggle")?.addEventListener("change", (ev) => _onProcessMessagesToggle(ev.target));
    return row;
  }
  ...
  async function _onStatusMessagesToggle(checkbox) {
    const desired = checkbox.checked;
    checkbox.disabled = true;
    try {
      await Channels.setStatusMessages(desired);
    } catch (e) {
      checkbox.checked = !desired;
      alert("Error: " + e.message);
    } finally {
  ...
  async function _onProcessMessagesToggle(checkbox) {
    const desired = checkbox.checked;
    checkbox.disabled = true;
    try {
      await Channels.setProcessMessages(desired);
    } catch (e) {
      checkbox.checked = !desired;
      alert("Error: " + e.message);
    } finally {
