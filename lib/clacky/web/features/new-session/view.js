...
const NewSessionView = (() => {
  ...
  let _modelsPromiseGeneration = -1;
  ...
  function _beginModelsRefresh() {
    _modelsLoaded = false;
    _modelsGeneration += 1;
    _renderContextBar();
    _updateSendButton();
    return _modelsGeneration;
  }

  async function _populateModels(generation = _modelsGeneration) {
    if (generation !== _modelsGeneration) return null;
    if (_modelsLoaded) return true;
    if (_modelsPromiseGeneration === generation) {
      if (_modelsPromise) return _modelsPromise;
    }

    const pending = (async () => {
      // Delay committing into shared store state until this display generation
      // is still current; an older response must not overwrite a later refresh.
      const models = await NewSessionStore.loadModels({ commit: false });
      if (generation !== _modelsGeneration) return null;
      if (!Array.isArray(models)) return false;
      NewSessionStore.setModels(models);
      _modelsLoaded = true;
    ...
    _modelsPromiseGeneration = generation;

    try {
      return await pending;
    } finally {
      if (_modelsPromise === pending) {
        _modelsPromise = null;
        _modelsPromiseGeneration = -1;
      }
    }
  }
  ...
  function _selectedModel() {
    return NewSessionStore.state.models.find(
      (model) => model.id === NewSessionStore.state.advanced.modelId
    );
  }

  function _selectedModelIsRuntime() {
    const model = _selectedModel();
    return !!(model && model.runtime_id);
  }

  function _stripInsertedSkillCommand(input = $("new-session-input")) {
    if (!input || typeof SkillAC === "undefined" ||
        typeof SkillAC.stripInsertedCommand !== "function") return false;
    return SkillAC.stripInsertedCommand(input);
  }
  ...
  function _updateSendButton() {
    ...
    const initProject = _modelsLoaded &&
      NewSessionStore.state.advanced.initProject === true &&
      !_selectedModelIsRuntime();
    const selectedModel = _selectedModel();
    const modelAvailable = !selectedModel || selectedModel.runtime_available !== false;
    btn.disabled = !_modelsLoaded || (!hasContent && !hasFiles && !initProject) ||
  ...
  function _openModelPopover(anchor) {
    _showCtxPopover(anchor, (pop) => {
      ...
      ModelPicker.populate(pop, {
        ...
        onSelect: (m) => {
          const patch = { modelId: m.id || "" };
          if (m.runtime_id) {
            patch.initProject = false;
            _stripInsertedSkillCommand();
          }
          NewSessionStore.updateAdvanced(patch);
          _dismissCtxPopover();
          _renderContextBar();
          _updateSendButton();
        },
  ...
  function _renderContextBar() {
    ...
    const runtimeModel = _selectedModelIsRuntime();
    const hostCommandsAvailable = _modelsLoaded && !runtimeModel;
    const slashButton = $("ns-btn-slash");
    if (slashButton) {
      slashButton.hidden = !hostCommandsAvailable;
      if (!hostCommandsAvailable) slashButton.classList.remove("active");
    }
    if (!hostCommandsAvailable) {
      const skillDropdown = $("ns-skill-autocomplete");
      if (skillDropdown) skillDropdown.style.display = "none";
      if (typeof SkillAC !== "undefined") SkillAC.hide();
    }
    ...
    if (agent && agent.id === "coding" && hostCommandsAvailable) {
  ...
  async function _submit() {
    ...
    const modelsReady = await _populateModels();
    if (!modelsReady) {
      alert(I18n.t("sessions.new.modelsUnavailable"));
      _renderContextBar();
      _updateSendButton();
      return;
    }

    if (_selectedModelIsRuntime()) _stripInsertedSkillCommand(input);
    ...
    if (!content && mentionChips.length === 0 && !hasFiles && !initProject) {
      _updateSendButton();
      return;
    }
  ...
  function _bindOnce() {
    ...
    if (typeof SkillAC !== "undefined" && input) {
      SkillAC.attach({
        ...
        isEnabled: () => _modelsLoaded && !_selectedModelIsRuntime(),
        fetchSkills: async () => {
          if (!_modelsLoaded || _selectedModelIsRuntime()) return [];
          const agent = NewSessionStore.currentAgent();
          if (!agent) return [];
  ...
  async function onPanelShow() {
    _bindOnce();
    const generation = _beginModelsRefresh();
    _loadAgentName().then(_renderTitle);
    await NewSessionStore.loadAgents();
    if (generation !== _modelsGeneration) return;
    const modelsReady = await _populateModels();
    if (generation !== _modelsGeneration) return;
    if (!modelsReady && typeof Modal !== "undefined") {
      Modal.toast(I18n.t("sessions.new.modelsUnavailable"), "error");
    }
