  ...
  function createViewer(onTabsChanged, onToggleTree, onShowMobileTree) {
    ...
    async function openTab(entry) {
      ...
      if (existing) {
        ...
        // A fallback pane renders nothing of its own — the handoff to the OS is
        // its entire effect, and it is not repeatable by redrawing. Re-selecting
        // the tab has to redo it, or a second click after the user closes the
        // document does nothing until the page is reloaded.
        if (existing.kind === "binary" && autoOpenWithSystem(entry.name)) {
          const opened = await openWithSystem(entry);
          if (opened && existing.subhintEl) existing.subhintEl.textContent = t("workspace.openedInSystem");
        }
        return;
      }
