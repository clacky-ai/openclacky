...
const WorkspaceView = (() => {
  ...
  // A file link is not consent to run or mount executable content.
  ...
  function blocksAutoOpen(filename) {
    const ext = (filename || "").split(".").pop().toLowerCase();
    return NO_AUTO_OPEN_EXTS.has(ext);
  }
  ...
  function createViewer(onTabsChanged, onToggleTree, onShowMobileTree) {
    ...
    async function openTab(entry, forceFallback) {
  ...
  function openFile(absPath, forceFallback) {
    ...
    _activeViewer.openTab(entry, forceFallback);
    revealTreePath(entry.path);
    return true;
  }

  async function openLinkedFile(absPath) {
    if (!absPath || !Workspace.state.hasSession()) return false;
    const entry = toEntry(absPath);
    if (CodeEditor.fileKind(entry.name) === "binary" && autoOpenWithSystem(entry.name)) {
      try {
        await Workspace.openWithDefault(entry);
        return true;
      } catch (err) {
    ...
    return openFile(absPath, blocksAutoOpen(entry.name));
  }

  // autoOpenWithSystem is exported for the regression harness.
  return { mount, openFile, openLinkedFile, autoOpenWithSystem };
})();
