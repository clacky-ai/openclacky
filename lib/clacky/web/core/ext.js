...
Clacky.ext = (() => {
  ...
  const ui = {
    ...
    // Persist only explicit user choices. Automatic sole-candidate resolution
    // deliberately stays ephemeral so a later second candidate triggers choice.
    selectHomepage(workspaceId) {
      const value = workspaceId === NATIVE_HOMEPAGE ? NATIVE_HOMEPAGE : workspaceId;
      if (value !== NATIVE_HOMEPAGE && !_homepageCandidates[value]) return false;
      try {
        window.localStorage.setItem(HOMEPAGE_STORAGE_KEY, value);
        return true;
      } catch (_error) {
        return false;
      }
    ...
    resolveHomepage() {
      ...
      let saved = null;
      try { saved = window.localStorage.getItem(HOMEPAGE_STORAGE_KEY); }
      catch (_error) { return { type: "native" }; }
