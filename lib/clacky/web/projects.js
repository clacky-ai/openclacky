...
const Projects = (() => {
   ...
   *  Pinned projects always come first (most recently pinned on top);
   *  the rest follow the selected sort mode. */
  ...
  function all()    { return _sortProjects(_projects); }
  ...
  function _renderFlat(container) {
    ...
    const projectSessions = allSessions
      ...
      .sort((a, b) => {
        ...
        // Secondary: within project, pinned first then newest
  ...
  function _renderGrouped(container) {
    ...
    sortedProjects.forEach(project => {
      const sessions = Sessions.all
        ...
        .sort((a, b) => {
          // Pinned sessions first, then newest first
          if (a.pinned && !b.pinned) return -1;
          if (!a.pinned && b.pinned) return 1;
          const ta = a.updated_at || a.created_at || "";
          const tb = b.updated_at || b.created_at || "";
      ...
      // Pin badge shown when the project is pinned
      if (project.pinned_at) {
        const pin = document.createElement("span");
        pin.className = "project-pin-badge";
        pin.title = I18n.t("projects.menu.unpin", "Unpin");
        pin.innerHTML = `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" xmlns="http://www.w3.org/2000/svg" style="transform:rotate(45deg);display:block"><line x1="12" y1="17" x2="12" y2="22"/><path d="M5 17h14v-1.76a2 2 0 0 0-1.11-1.79l-1.78-.9A2 2 0 0 1 15 10.76V6h1a2 2 0 0 0 0-4H8a2 2 0 0 0 0 4h1v4.76a2 2 0 0 1-1.11 1.79l-1.78.9A2 2 0 0 0 5 15.24Z"/></svg>`;
  ...
  function _showProjectMenu(projectId, anchor) {
    ...
    // Lucide icons, embedded to avoid extra asset requests.
    ...
    const iconEdit = `<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 1 1 3 3L7 19l-4 1 1-4z"/></svg>`;
    const iconClearSessions = `<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="m16 22-1-4"/><path d="M19 14a1 1 0 0 0 1-1v-1a2 2 0 0 0-2-2h-3a1 1 0 0 1-1-1V4a2 2 0 0 0-4 0v5a1 1 0 0 1-1 1H6a2 2 0 0 0-2 2v1a1 1 0 0 0 1 1"/><path d="M19 14H5l-1.973 6.767A1 1 0 0 0 4 22h16a1 1 0 0 0 .973-1.233z"/><path d="m8 22 1-4"/></svg>`;
    const iconTrash = `<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M3 6h18"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6"/><path d="M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>`;
    ...
    const isPinned = !!(project && project.pinned_at);
    const pinToggle = async () => {
      try {
        await update(projectId, { pinned: !isPinned });
        renderSection();
      } catch (err) {
    ...
    addItem("projects.menu.deleteSessions", "Delete Sessions", iconClearSessions, false, () => promptDeleteSessions(projectId));
...
<div id="create-project-modal-overlay" class="modal-overlay" style="display:none">
  ...
  function init() {
    ...
    document.addEventListener("langchange", () => renderSection());
  }
