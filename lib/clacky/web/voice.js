...
const Voice = (() => {
  ...
  let _lastDisplay  = "";     // what we last wrote to the input — used to detect manual edits
  let _btn          = null;
  ...
  function _emitResult(text, isSentenceEnd) {
    if (!text) return;

    // Absorb manual edits BEFORE processing — otherwise processAsrResult
    // runs against stale _asrState and the first display flashes old content.
    _absorbManualEdits();

    if (typeof processAsrResult !== "undefined") {
      _asrState = processAsrResult(_asrState || createInitialState(), text, isSentenceEnd);
            ...
            const runTask = {
              ...
              payload: {
                ...
                parameters: {
                  format: "pcm",
                  sample_rate: 16000,
                  language_hints: [ (_cfg("language", "en-US") || "en-US").split("-")[0], "en" ]
                },
                input: {}
              }
            };
        ...
        ws.onmessage = (event) => {
          ...
          try {
            ...
            if (action === "result-generated") {
              ...
              // end_time === 0 → interim; end_time > 0 → sentence complete (final)
              const isEnd = (sentence.end_time != null) ? sentence.end_time > 0 : undefined;
              _emitResult(text, isEnd);
  ...
  function _absorbManualEdits() {
    const input = _getInput();
    if (!input) return;
    const current = input.value;
    if (current !== _lastDisplay && _asrState) {
      _asrState.interim = "";
      _asrState.queue = current;
      if (current.length > (_asrState.maxLen || 0)) {
        _asrState.maxLen = current.length;
      }
    }
  }
  ...
  function _finalizeCurrent() {
    ...
    // Only reset accumulated ASR state in voice mode (hands-free auto-send).
    // In push-to-talk mode, keep _asrState so multi-segment input accumulates
    // across silence gaps — the user controls when to send.
    if (_voiceMode) {
  ...
  async function start() {
    ...
    // Push-to-talk: preserve any unsent text from previous recording,
    // so multi-segment voice input accumulates. Voice mode always starts fresh.
    if (_voiceMode) {
      _updateInput("");
    } else {
      const input = _getInput();
      ...
      // else: keep existing text — will be carried into _asrState.queue below
    }
    ...
    if (!ok) {
      _engine = null;
      // If browser native engine failed because the API is unavailable,
      // show a user-friendly toast guiding them to configure DashScope.
      const provider = _cfg("asr.provider", "google");
      if (provider === "google" && typeof Modal !== "undefined" && Modal.toast) {
        Modal.toast(I18n.t("settings.voice.unsupported"), "error");
      }
      return;
    }

    // Restore asrState: in push-to-talk, preserve existing text as queue
    if (!_voiceMode) {
      const input = _getInput();
      const existing = input ? input.value.trim() : "";
      if (existing && typeof createInitialState !== "undefined") {
        _asrState = { interim: "", queue: existing, maxLen: existing.length };
      } else {
        _asrState = (typeof createInitialState !== "undefined") ? createInitialState() : null;
      }
    } else {
  ...
  async function init() {
    ...
    // Apply default_mode from config (push-to-talk is the built-in default).
    // "hands-free" pre-selects the mode but does NOT auto-start recording;
    // the user still needs to click the mic button to begin.
    if (_cfg("default_mode", "push-to-talk") === "hands-free") {
      _voiceMode = true;
    }
  }
  ...
  async function reloadConfig() {
    await _loadVoiceConfig();
    // Re-apply default_mode if not currently in voice mode
    if (!_voiceMode && _cfg("default_mode", "push-to-talk") === "hands-free") {
      _voiceMode = true;
      _updateBtnUI();
    } else if (_voiceMode && _cfg("default_mode", "push-to-talk") !== "hands-free") {
      _voiceMode = false;
      _updateBtnUI();
    }
  }
