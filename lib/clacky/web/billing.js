...
const Billing = (() => {
  ...
  async function _load() {
    ...
    if (isFirstLoad) {
      ...
      // Keep existing DOM intact (preserves Tab bar), just dim the stats area
      const existing = container.querySelector(".billing-dashboard");
      if (existing) existing.classList.add("billing-loading-overlay");
    }
  ...
  function _render() {
    ...
    // Remove loading overlay if present (set during Tab switch)
    const existing = container.querySelector(".billing-dashboard");
    if (existing) existing.classList.remove("billing-loading-overlay");
