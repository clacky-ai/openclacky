...
const SkillAC = (() => {
  ...
  let _renderRequest   = 0;   // fences async results across composer changes
  let _snapshotRequest = 0;   // fences background session-snapshot loads
  const _insertedCommands = new WeakMap(); // composer -> SkillAC-inserted skill id
  ...
  const _chatCfg = {
    ...
    isEnabled: () => {
      const session = Sessions.find(Sessions.activeId);
      return !session || !session.runtime_id;
    },
  ...
  function _isEnabled(cfg = _cfg) {
    return !cfg.isEnabled || cfg.isEnabled() !== false;
  }
  ...
  function _loadForSession(sessionId) {
    ...
    if (_currentSession) {
      _fetchSkills(_chatCfg, _currentSession).then(skills => {
        if (request === _snapshotRequest &&
            _currentSession === sessionId && _cfg === _chatCfg) {
          _skillsSnapshot = skills;
        }
      }).catch(() => {});
    }
  }
  ...
  async function _fetchSkills(cfg = _cfg, sessionId = _currentSession) {
    if (!_isEnabled(cfg)) return [];
    if (cfg.fetchSkills) {
      try {
        return (await cfg.fetchSkills()) || [];
      } catch (e) {
  ...
  function _renderUserMessageHtml(content, skillCommand, skillCommandDisplay) {
    ...
    return `<span class="msg-slash-cmd">${escapeHtml("/" + display)}</span>${escapeHtml(text.slice(token.length))}`;
  }

  async function _render(query) {
    const cfg = _cfg;
    const sessionId = _currentSession;
    const request = ++_renderRequest;
    if (!_isEnabled(cfg)) { _hide(); return; }
    const all = await _fetchSkills(cfg, sessionId);
    if (request !== _renderRequest || _cfg !== cfg ||
        _currentSession !== sessionId || !_isEnabled(cfg)) return;
  ...
  function _select(idx) {
    ...
    _insertedCommands.set(input, skill.name);
    _hide();
    input.focus();
  }

  function _insertedCommandRemainder(input) {
    const name = input && _insertedCommands.get(input);
    if (!name) return null;

    const value = Composer.text(input);
    const prefix = "/" + name;
    if (value === prefix) return "";
    if (!value.startsWith(prefix) || !/^\s/.test(value.slice(prefix.length))) return null;
    return value.slice(prefix.length).replace(/^\s+/, "");
  }

  function _syncInsertedCommand(input) {
    if (input && _insertedCommands.has(input) && _insertedCommandRemainder(input) === null) {
      _insertedCommands.delete(input);
    }
  }

  // Remove only a command inserted through this autocomplete. Manually typed
  // slash text is never touched, and any description after the command stays.
  function _stripInsertedCommand(input) {
    const remainder = _insertedCommandRemainder(input);
    if (input) _insertedCommands.delete(input);
    if (remainder === null) return false;

    Composer.setText(input, remainder);
    return true;
  }
  ...
  async function _openAll() {
    if (!_isEnabled()) { _hide(); return; }
    const cfg = _cfg;
    _activeIndex = 0;  // Default to first item
    await _render("");
    if (_cfg === cfg && _isEnabled(cfg)) $(cfg.input).focus();
  }
  ...
  return {
    ...
    /** Strip a command previously inserted by SkillAC, preserving its description. */
    stripInsertedCommand: _stripInsertedCommand,
