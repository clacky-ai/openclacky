...
const SkillAC = (() => {
  ...
  function _renderFileMentions(text) {
    ...
    return escapeHtml(text).replace(/(^|[^A-Za-z0-9._%+-])(@"(?:[^"\\]|\\.)*"?|@[^\s@]+)/g, (_m, boundary, mention) => {
      let body = mention.slice(1); // drop the leading @
      if (body.startsWith('"') && body.endsWith('"') && body.length >= 2) body = body.slice(1, -1);
      return `${boundary}<span class="file-mention-tag">@${body}</span>`;
    });
  }
