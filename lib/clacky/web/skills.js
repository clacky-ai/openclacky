...
const Skills = (() => {
  ...
  async function _handleSkillUpload(file) {
    if (!file) return;
    ...
    try {
      const res  = await fetch("/api/my-skills/upload", { method: "POST", body: formData });
      const data = await res.json();
      ...
      if (!res.ok || !data.ok) {
        ...
        if (statusEl) {
          statusEl.textContent = `✓ Skill "${data.name}" uploaded successfully!`;
          statusEl.className   = "upload-skill-status upload-ok";
        }
        // Reload skills to include newly uploaded skill
        await Skills.load();
