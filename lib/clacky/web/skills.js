...
const SkillAC = (() => {
  ...
  /** Full-width slash / Chinese dunhao typed by an IME count as a leading "/". */
  function _normalizeSlash(value) {
    return value.replace(/^[／、]/, "/");
  }

  /**
   * Return the command name of a slash command, or null for a plain message.
   * Mirrors the backend's parse_skill_command (agent/skill_manager.rb) — keep the
   * two in sync, a client that is more permissive pops the palette over text the
   * server treats as an ordinary message.
   *
   * Rejected: no leading "/", an empty first token ("/"), whitespace right after
   * the "/", and any first token containing a second "/" — that shape is a
   * filesystem path ("/Users/alice/foo"), not a command. Non-slug names such as
   * "/guizang-ppt-skill:create" are deliberately accepted; resolving them is the
   * skill lookup's job, not the parser's.
   */
  function _slashCommandName(value) {
    const text = _normalizeSlash(String(value == null ? "" : value));
  ...
  /** Return the /xxx prefix if the input is a slash command being typed, else null. */
  function _getSlashQuery(value) {
    // Normalization also happens in the input event handler, but guard here too
    // in case the value is passed programmatically.
    const text = _normalizeSlash(String(value == null ? "" : value));

    // A bare "/" has no name yet — it opens the full palette.
    if (text === "/") return "";
    // Whitespace means the user moved on to arguments; stop tracking the name.
    if (/\s/.test(text)) return null;

    const name = _slashCommandName(text);
  ...
  function _renderUserMessageHtml(content, skillCommand, skillCommandDisplay) {
    ...
    return `<span class="msg-slash-cmd">${escapeHtml("/" + display)}</span>${escapeHtml(text.slice(name.length + 1))}`;
  }
  ...
  function _handleKey(e) {
    ...
    if (e.key === "Enter" && !_ime.isComposing(e)) {
      // The dropdown stays visible on an empty result set to show its empty
      // state, so guard on a real item — never swallow Enter with nothing to pick.
      if (_items[_activeIndex]) {
        e.preventDefault();
  ...
  return {
    ...
    /** Command name of a slash command, or null when the text is a plain message. */
    slashCommandName: _slashCommandName,
