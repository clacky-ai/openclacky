...
const Skills = (() => {
    ...
    _storeSkills.forEach(item => {
      ...
      if (!installed) {
        card.querySelector(".btn-store-install").addEventListener("click", () => {
          Skills.installFromStore(slug, title, downloadUrl);
        });
      }
  ...
  return {
     ...
     */
    async installFromStore(name, title, downloadUrl) {
      ...
      const msg = `/skill-add Install the "${title}" skill from ${downloadUrl}`;
