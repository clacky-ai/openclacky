...
(function() {
  ...
  // A phase is spawned by a tool call, so its card belongs inside that tool's
  // row — otherwise a fan-out of N subagents drops N sibling cards into the
  // main stream and buries the conversation. Falls back to the outer stream
  // for phases with no tool behind them (e.g. memory_update).
  //
  // The anchor is resolved from the DOM rather than Sessions._liveLastToolItem:
  // a subagent's own tool calls overwrite that pointer, which would nest the
  // second fan-out card inside the first job instead of beside it.
  function _phaseHost() {
    ...
    for (let i = running.length - 1; i >= 0; i--) {
      const candidate = running[i].closest(".tool-item");
      if (candidate && !candidate.parentElement.closest(".msg-phase-body")) {
        item = candidate;
        break;
      }
    }
    ...
    case "subscribed": {
      ...
      if (pendingMsg && pendingMsg.session_id === ev.session_id) {
        const html = pendingMsg.files && pendingMsg.files.length
          ? (pendingMsg.display || SkillAC.renderUserMessageHtml(pendingMsg.content))
          : SkillAC.renderUserMessageHtml(pendingMsg.display || pendingMsg.content);
