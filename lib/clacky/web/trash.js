...
const Trash = (() => {
  ...
  async function _restoreSession(session, card) {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok || !data.ok) {
        alert(I18n.t("trash.sessionRestoreFail", {
          msg: data.error || res.statusText
        }));
      } else {
        ...
        _renderSessions();
        // Refresh the sidebar session list so the restored session appears.
        if (typeof Sessions !== "undefined") Sessions.renderList();
      }
    } catch (e) {
      alert(I18n.t("trash.sessionRestoreFail", { msg: e.message }));
    } finally {
      btn.disabled = false;
    }
  }
  ...
  async function _emptyBulk(daysOld, confirmKey) {
    ...
    if (_activeTab === "session-trash") {
      return _emptySessionsBulk(daysOld);
    }
  ...
  async function _emptySessionsBulk(daysOld) {
    ...
    try {
      ...
      alert(I18n.t("trash.sessionsCleaned", {
        count: data.deleted_count || 0
      }));
      await _loadSessions();
    } catch (e) {
