...
const Settings = (() => {
  ...
  // Group selected in the model edit modal (null = ungrouped).
  let _modalSelectedGroup = null;
  // Grouping UI state: active filter tab ("all" or a group name), collapsed
  // groups, empty groups created in this session (groups are stored on the
  // model records, so empty ones cannot be persisted — a known trade-off
  // documented in issue #481), and inline input row modes.
  let _activeGroupTab = "all";
  let _collapsedGroups = new Set();
  let _extraGroups = new Set();
  let _addingGroup = false;
  let _renamingGroup = null;
  ...
  function _renderCards() {
    ...
    if (_models.length === 0 && _extraGroups.size === 0) {
      container.innerHTML = `<div class="settings-empty">${I18n.t("settings.models.empty")}</div>`;
      return;
    }

    // Filter tabs: All + one tab per named group.
    container.appendChild(_renderGroupTabs());

    // Inline "add group" input row (toggled by the "+ Add Group" button).
    if (_addingGroup) container.appendChild(_renderGroupInputRow(null));

    // Inline "rename group" input row (toggled by a group's rename button).
    if (_renamingGroup !== null) container.appendChild(_renderGroupInputRow(_renamingGroup));

    const groups = _groupNames();
    if (_activeGroupTab !== "all" && !groups.includes(_activeGroupTab)) {
      _activeGroupTab = "all"; // tab target vanished (group deleted); fall back
    }

    if (_activeGroupTab === "all") {
      groups.forEach(g => container.appendChild(_renderGroup(g)));
      // Ungrouped models always visible in the "All" view.
      container.appendChild(_renderGroup(null));
    } else {
      container.appendChild(_renderGroup(_activeGroupTab));
    }
  }

  function _groupNames() {
    const names = new Set(_extraGroups);
    _models.forEach(m => { if (m.group) names.add(m.group); });
    return [...names].sort((a, b) => a.localeCompare(b));
  }

  function _renderGroupTabs() {
    const tabs = document.createElement("div");
    tabs.className = "model-group-tabs";
    const addTab = (label, key) => {
      const b = document.createElement("button");
      b.type = "button";
      b.className = "model-group-tab" + (_activeGroupTab === key ? " active" : "");
      b.textContent = label;
    ...
    addTab(I18n.t("settings.models.group.all"), "all");
    _groupNames().forEach(g => addTab(_esc(g), g));
    return tabs;
  }

  function _renderGroup(name) {
    const isUngrouped = name === null;
    const members = isUngrouped
      ? _models.filter(m => !m.group)
      : _models.filter(m => m.group === name);
    const displayName = isUngrouped ? I18n.t("settings.models.group.ungrouped") : name;
    const collapsed = _collapsedGroups.has(name === null ? "__ungrouped__" : name);
    ...
    head.innerHTML = `
      <button type="button" class="model-group-collapse" title="${isUngrouped ? "" : _esc(displayName)}">
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="transform:${collapsed ? "rotate(-90deg)" : "none"};transition:transform .15s"><polyline points="6 9 12 15 18 9"/></svg>
      </button>
      <span class="model-group-name">${_esc(displayName)}</span>
      <span class="model-group-count">${members.length}</span>
      ${!isUngrouped ? `
        <span class="model-group-tools">
          <button type="button" class="btn-card-grid-action btn-card-grid-action-primary" data-group="${_esc(name)}" data-gaction="add">${I18n.t("settings.models.group.addModel")}</button>
          <button type="button" class="btn-card-grid-action" data-group="${_esc(name)}" data-gaction="rename">${I18n.t("settings.models.group.rename")}</button>
          <button type="button" class="btn-card-grid-action btn-card-grid-action-danger" data-group="${_esc(name)}" data-gaction="delete">${I18n.t("settings.models.group.delete")}</button>
        </span>` : ""}
    `;
    head.querySelector(".model-group-collapse").addEventListener("click", () => {
    ...
    head.querySelectorAll("[data-gaction]").forEach(btn => {
      btn.addEventListener("click", () => {
        const g = btn.dataset.group;
        if (btn.dataset.gaction === "rename") { _renamingGroup = g; _renderCards(); }
        else if (btn.dataset.gaction === "add") { _openGroupAddModal(g); }
        else _deleteGroup(g);
      });
    });
    ...
    if (members.length === 0) {
      body.innerHTML = `<div class="settings-empty model-group-empty">${I18n.t("settings.models.group.empty")}</div>`;
    } else {
      members.forEach(m => _renderCard(body, m, _models.indexOf(m)));
    }
    wrap.appendChild(body);
    return wrap;
  }

  function _renderGroupInputRow(targetName) {
    const row = document.createElement("div");
    row.className = "model-group-input-row";
    const isRename = targetName !== null;
    row.innerHTML = `
      <input type="text" class="field-input" id="model-group-input" maxlength="50"
             placeholder="${_esc(I18n.t("settings.models.group.name"))}"
             value="${isRename ? _esc(targetName) : ""}" autocomplete="off">
      <button type="button" class="btn-primary" id="model-group-input-save">${isRename ? I18n.t("settings.models.btn.save") : I18n.t("settings.models.group.addBtn")}</button>
      <button type="button" class="btn-secondary" id="model-group-input-cancel">${I18n.t("settings.models.btn.cancel")}</button>
    `;
    const input = row.querySelector("#model-group-input");
    const doSave = () => {
      const name = input.value.trim();
      if (!name) { input.focus(); return; }
      if (isRename) _renameGroup(targetName, name);
      else _addGroup(name);
    };
    row.querySelector("#model-group-input-save").addEventListener("click", doSave);
    row.querySelector("#model-group-input-cancel").addEventListener("click", () => {
      _addingGroup = false; _renamingGroup = null; _renderCards();
    });
  ...
  function _addGroup(name) {
    if (_groupNames().includes(name) || _models.some(m => m.group === name)) {
      alert(I18n.t("settings.models.group.duplicate", { name }));
      return;
    }
    _extraGroups.add(name);
    _addingGroup = false;
    _activeGroupTab = name;
    _renderCards();
  }

  async function _renameGroup(oldName, newName) {
    if (_groupNames().filter(n => n !== oldName).includes(newName) ||
        _models.some(m => m.group === newName)) {
      alert(I18n.t("settings.models.group.duplicate", { name: newName }));
      return;
    }
    const members = _models.filter(m => m.group === oldName);
    try {
      await Promise.all(members.map(m =>
        ModelTester.saveModel({ group: newName }, { existingId: m.id })
      ));
      members.forEach(m => { m.group = newName; });
      _extraGroups.delete(oldName);
      if (members.length === 0) _extraGroups.add(newName);
      if (_activeGroupTab === oldName) _activeGroupTab = newName;
      _renamingGroup = null;
      _renderCards();
    } catch (_) {
      alert(I18n.t("settings.models.saveFailed"));
      _renamingGroup = null;
      _renderCards();
    }
  }

  async function _deleteGroup(name) {
    const members = _models.filter(m => m.group === name);
    if (!(await Modal.confirm(I18n.t("settings.models.group.confirmDelete", { name })))) return;
    try {
      await Promise.all(members.map(m =>
        ModelTester.saveModel({ group: "" }, { existingId: m.id })
      ));
      members.forEach(m => { m.group = null; });
      _extraGroups.delete(name);
      if (_activeGroupTab === name) _activeGroupTab = "all";
      _renderCards();
    } catch (_) {
      alert(I18n.t("settings.models.saveFailed"));
      _load();
  ...
  function _openGroupAddModal(groupName) {
    // Candidates: every model not already in this group (ungrouped + others).
    const candidates = _models.filter(m => (m.group || null) !== groupName);
    ...
    const rows = candidates.map((m, i) => `
      <label class="model-picker-row">
        ...
        <span class="model-picker-name">${_esc(m.model || I18n.t("settings.models.unnamed"))}</span>
        <span class="model-picker-group-tag">${m.group ? _esc(m.group) : I18n.t("settings.models.group.ungrouped")}</span>
      </label>
    `).join("");

    overlay.innerHTML = `
      <div class="modal-content model-group-add-content">
        <div class="modal-header">
          <h3>${I18n.t("settings.models.group.addTo", { name: _esc(groupName) })}</h3>
          <button type="button" class="modal-close" data-gamodal="close" aria-label="Close">
            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
          </button>
        </div>
        <div class="modal-body">
          ${candidates.length === 0
            ? `<div class="settings-empty model-group-empty">${I18n.t("settings.models.group.noCandidates")}</div>`
            : `
              <label class="model-picker-select-all">
                <input type="checkbox" id="model-picker-select-all">
                <span>${I18n.t("settings.models.group.selectAll")}</span>
              </label>
              <div class="model-picker-list">${rows}</div>
            `}
        </div>
        <div class="modal-footer">
          <button type="button" class="btn-secondary" data-gamodal="cancel">${I18n.t("settings.models.btn.cancel")}</button>
          <button type="button" class="btn-primary" data-gamodal="confirm" disabled>${I18n.t("settings.models.group.addModel")}</button>
        </div>
      </div>
    `;
    ...
    overlay.addEventListener("click", e => {
      ...
      const btn = e.target.closest("[data-gamodal]");
      if (btn && btn.dataset.gamodal === "close") close();
      if (btn && btn.dataset.gamodal === "cancel") close();
    });

    if (candidates.length === 0) return;

    const boxes = [...overlay.querySelectorAll(".model-picker-row input[type=checkbox]")];
    const selectAll = overlay.querySelector("#model-picker-select-all");
    const confirmBtn = overlay.querySelector("[data-gamodal=confirm]");

    const refresh = () => {
      const n = boxes.filter(b => b.checked).length;
      confirmBtn.disabled = n === 0;
      confirmBtn.textContent = `${I18n.t("settings.models.group.addModel")} (${n})`;
      if (selectAll) selectAll.checked = n === boxes.length;
    };
    ...
    confirmBtn.addEventListener("click", async () => {
      const selected = candidates.filter((_, i) => boxes[i].checked);
      if (selected.length === 0) return;
      confirmBtn.disabled = true;
      confirmBtn.textContent = I18n.t("settings.models.btn.saving");
      try {
        await Promise.all(selected.map(m =>
          ModelTester.saveModel({ group: groupName }, { existingId: m.id })
        ));
        selected.forEach(m => { m.group = groupName; });
        _extraGroups.add(groupName);
        close();
        alert(I18n.t("settings.models.group.added", { n: selected.length, name: groupName }));
        _renderCards();
      } catch (_) {
        confirmBtn.disabled = false;
        confirmBtn.textContent = I18n.t("settings.models.group.addModel");
        alert(I18n.t("settings.models.saveFailed"));
      }
    });
  }
  ...
  // The remark input is only offered for custom (non-preset) providers: it is
  // shown once the dropdown actually reads "Custom", hidden otherwise. The
  // typed value is kept so toggling providers never silently drops it.
  function _syncRemarkField() {
    const valueEl = document.getElementById("model-modal-provider-value");
    const input = document.getElementById("model-modal-remark");
    if (!valueEl || !input) return;
    const isCustom = !valueEl.classList.contains("placeholder") && !_modalSelectedProviderId;
    const field = input.closest(".model-field");
    if (field) field.style.display = isCustom ? "" : "none";
  }
  ...
  function _openModal(index = -1) {
    ...
    if (index >= 0 && _models[index]) {
      ...
      _modalSelectedGroup = model.group || null;
      _populateModalGroupDropdown();
      const groupValue = document.getElementById("model-modal-group-value");
      groupValue.textContent = _modalSelectedGroup || I18n.t("settings.models.group.ungrouped");
      groupValue.classList.remove("placeholder");
    } else {
      // Add mode
      ...
      // Reset group dropdown (new models start ungrouped).
      _modalSelectedGroup = null;
      _populateModalGroupDropdown();
      const groupValue = document.getElementById("model-modal-group-value");
      groupValue.textContent = I18n.t("settings.models.group.ungrouped");
      groupValue.classList.remove("placeholder");
    }
  ...
  function _populateModalGroupDropdown() {
    const dropdown = document.getElementById("model-modal-group-dropdown");
    dropdown.innerHTML = `
      <div class="custom-select-option" data-group="">${I18n.t("settings.models.group.ungrouped")}</div>
      ${_groupNames().map(g => `<div class="custom-select-option" data-group="${_esc(g)}">${_esc(g)}</div>`).join("")}
    `;
    dropdown.querySelectorAll(".custom-select-option").forEach(option => {
      option.classList.toggle("selected", (_modalSelectedGroup || "") === option.dataset.group);
      option.addEventListener("click", (e) => {
        e.stopPropagation();
        _modalSelectedGroup = option.dataset.group || null;
        const groupValue = document.getElementById("model-modal-group-value");
        groupValue.textContent = option.dataset.group || I18n.t("settings.models.group.ungrouped");
        groupValue.classList.remove("placeholder");
        dropdown.classList.remove("open");
        document.getElementById("model-modal-group-trigger").classList.remove("open");
      });
    });
  }
  ...
  async function _saveModalModel() {
    ...
    // Group is persisted as "" when unset so PATCH clears a previous value.
    payload.group = _modalSelectedGroup || "";
  ...
  function _initModal() {
    ...
    // Group dropdown toggle (options are rebuilt each time so freshly
    // created groups show up without reopening the modal).
    const groupTrigger = document.getElementById("model-modal-group-trigger");
    const groupDropdown = document.getElementById("model-modal-group-dropdown");
    groupTrigger.addEventListener("click", (e) => {
      ...
      if (!isOpen) {
        ...
        groupTrigger.classList.add("open");
      } else {
        groupDropdown.classList.remove("open");
        groupTrigger.classList.remove("open");
      }
    });
