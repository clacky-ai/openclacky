...
const Settings = (() => {
  ...
  async function _activateLicense() {
    ...
    try {
      ...
      if (data.ok) {
        ...
        // Show a prompt to go check brand skills
        setTimeout(_showBrandSkillsPrompt, 1000);
      } else {
  ...
  // Show a prompt after successful activation to guide users to brand skills.
  function _showBrandSkillsPrompt() {
    const result = document.getElementById("settings-activate-result");
    if (!result) return;

    // Remove any existing prompt first (idempotent)
    const existing = document.getElementById("brand-skills-prompt");
    if (existing) existing.remove();

    const prompt = document.createElement("div");
    prompt.id        = "brand-skills-prompt";
    prompt.className = "brand-skills-prompt";

    const text = document.createElement("span");
    text.setAttribute("data-i18n", "settings.brand.skillsPrompt");
    text.textContent = I18n.t("settings.brand.skillsPrompt");

    const btn = document.createElement("button");
    btn.className = "btn-brand-skills-goto";
    btn.setAttribute("data-i18n", "settings.brand.skillsBtn");
    btn.textContent = I18n.t("settings.brand.skillsBtn");
    btn.addEventListener("click", () => {
      // Close settings and open the Skills panel on the brand-skills tab
      Router.navigate("skills");
      Skills.openBrandSkillsTab();
    });

    prompt.appendChild(text);
    prompt.appendChild(btn);
    result.insertAdjacentElement("afterend", prompt);
  }
