...
const Settings = (() => {
  ...
  // API protocol selected in the model edit modal ("auto" by default).
  // "auto" lets the backend resolve based on provider preset + model name.
  let _modalApiProtocol = "auto";
  ...
  function _openModal(index = -1) {
    ...
    if (index >= 0 && _models[index]) {
      ...
      // Set API protocol dropdown from saved value (default "auto")
      _modalApiProtocol = model.api_protocol || "auto";
      _updateProtocolOptions(resolvedProviderId);
      document.getElementById("model-modal-api-protocol").value = _modalApiProtocol;
      ...
      // Reset API protocol to "auto"
      _modalApiProtocol = "auto";
      _updateProtocolOptions(null);
      document.getElementById("model-modal-api-protocol").value = "auto";
    }
  ...
  function _openModalDuplicate(index) {
    ...
    // Copy API protocol from source model
    _modalApiProtocol = source.api_protocol || "auto";
    _updateProtocolOptions(_modalSelectedProviderId);
    document.getElementById("model-modal-api-protocol").value = _modalApiProtocol;
    _syncRemarkField();
  }

  // Update the API protocol dropdown options based on the selected provider.
  // When a provider has a known set of supported protocols, restrict the
  // dropdown to only those options. "auto" is always available.
  function _updateProtocolOptions(providerId) {
    const select = document.getElementById("model-modal-api-protocol");
    if (!select) return;

    const provider = providerId ? _providers.find(p => p.id === providerId) : null;
    const supported = provider && provider.supported_protocols ? provider.supported_protocols : null;

    // All possible options
    const allOptions = [
      { value: "auto", label: I18n.t("settings.models.apiProtocol.auto") || "Auto (detect from provider)" },
      { value: "openai-completions", label: "OpenAI Chat Completions" },
      { value: "openai-responses", label: "OpenAI Responses API" },
      { value: "anthropic-messages", label: "Anthropic Messages API" }
    ];

    // Filter to supported protocols if available, otherwise show all
    const options = supported
      ? allOptions.filter(o => o.value === "auto" || supported.includes(o.value))
      : allOptions;

    const currentValue = _modalApiProtocol || "auto";
    select.innerHTML = options.map(o => `<option value="${o.value}">${o.label}</option>`).join("");

    // Preserve current selection if still valid, otherwise fall back to "auto"
    select.value = options.some(o => o.value === currentValue) ? currentValue : "auto";
    _modalApiProtocol = select.value;
  }
      ...
      option.addEventListener("click", (e) => {
        ...
        if (value && value !== "custom") {
          ...
          if (preset) {
            ...
            // Update protocol options based on provider's supported protocols
            _updateProtocolOptions(value);
          }
        }
      });
    });
  ...
  async function _testModel(index) {
    ...
    try {
      const result = await ModelTester.testConnection({
        ...
        api_protocol: model.api_protocol || "auto",
  ...
  async function _saveModalModel() {
    ...
    // API protocol: read from the modal dropdown. "auto" delegates to
    // backend resolution (provider preset + model name overrides).
    _modalApiProtocol = document.getElementById("model-modal-api-protocol").value || "auto";
    ...
    const result = await ModelTester.testConnection({
      model, base_url, api_key, index, id: existingId || sourceId, api_protocol: _modalApiProtocol, anthropic_format
    });
    ...
    const payload = { model, base_url, api_protocol: _modalApiProtocol, anthropic_format, provider_id: _modalSelectedProviderId || null };
  ...
  function _readCard(index) {
    ...
    return {
      ...
      api_protocol:    _models[index]?.api_protocol || "auto",
