...
const Settings = (() => {
    ...
    card.innerHTML = `
      <div class="model-card-grid-info">
        <div class="model-card-grid-name-row">
          ...
          ${isImage ? `<span class="badge badge-media">${I18n.t("settings.models.badge.image")}</span>` : ""}
          ${isVideo ? `<span class="badge badge-media">${I18n.t("settings.models.badge.video")}</span>` : ""}
          ${isAudio ? `<span class="badge badge-media">${I18n.t("settings.models.badge.audio")}</span>` : ""}
        </div>
  ...
  function _openModal(index = -1) {
    ...
    if (index >= 0 && _models[index]) {
      ...
      document.getElementById("model-modal-role").value = model.type || "";
      ...
      document.getElementById("model-modal-default-field").style.display = "";
      document.getElementById("model-modal-role").value = "default";
  ...
  async function _saveModalModel() {
    ...
    const role = document.getElementById("model-modal-role").value || null;
    payload.type = role;
    if (role) {
      _models.forEach((m, i) => {
        if (i !== index && m.type === role) m.type = null;
      });
    }
