...
const Settings = (() => {
  ...
  function _detectBrowserSpeechSupport() {
    return !!(window.SpeechRecognition || window.webkitSpeechRecognition);
  }

  function _renderVoiceSettings() {
    const asr = (_voiceConfigCache && _voiceConfigCache.asr) || {};
    const provider = asr.provider || "google";
    const maskedKey = asr.api_key_masked || "";

    // Engine selection
    const engineBtns = document.querySelectorAll("#voice-engine-select .voice-engine-btn");
    engineBtns.forEach(btn => {
      btn.classList.toggle("is-active", btn.dataset.engine === provider);
    });

    // API key input
    const apikeyInput = document.getElementById("voice-apikey-input");
    if (apikeyInput) apikeyInput.value = maskedKey;

    _updateVoiceEngineUI(provider);

    // Language
    const langBtns = document.querySelectorAll("#voice-language-select .settings-lang-btn");
    const savedLang = _voiceConfigCache.language || "en-US";
    langBtns.forEach(btn => {
      btn.classList.toggle("active", btn.dataset.lang === savedLang);
    });

    // Default mode
    const mode = _voiceConfigCache.default_mode || "push-to-talk";
    const modeBtns = document.querySelectorAll("#voice-mode-select .voice-engine-btn");
    modeBtns.forEach(btn => {
      btn.classList.toggle("is-active", btn.dataset.mode === mode);
    });

    // Behavior
    const silenceInput = document.getElementById("voice-silence-timeout");
    if (silenceInput) silenceInput.value = _voiceConfigCache.silence_timeout_ms ?? "";
    const restartInput = document.getElementById("voice-restart-delay");
    if (restartInput) restartInput.value = _voiceConfigCache.voice_mode_restart_delay_ms ?? "";

    // Shortcuts
    const shortcuts = _voiceConfigCache.shortcuts || {};
    ["toggle", "stop", "start", "voice_mode"].forEach(action => {
      const input = document.getElementById("voice-shortcut-" + action + "-key");
      if (input) input.value = (shortcuts[action] && shortcuts[action].key) || "";
    });

    // Exit words
    const exitWordsInput = document.getElementById("voice-exit-words");
    if (exitWordsInput) {
      const words = _voiceConfigCache.exit_words || [];
      exitWordsInput.value = Array.isArray(words) ? words.join(", ") : "";
    }

    // Sound volume
    const volumeInput = document.getElementById("voice-sound-volume");
    const volumeValue = document.getElementById("voice-sound-volume-value");
    const vol = (_voiceConfigCache.sound && _voiceConfigCache.sound.volume) ?? 0.4;
    if (volumeInput) volumeInput.value = vol;
    if (volumeValue) volumeValue.textContent = vol;
  }

  function _updateVoiceEngineUI(engine) {
    const apikeyRow = document.getElementById("voice-apikey-row");
    const apikeyHint = document.getElementById("voice-apikey-hint");
    const browserHint = document.getElementById("voice-browser-hint");

    if (engine === "dashscope") {
      if (apikeyRow) apikeyRow.style.display = "";
      if (apikeyHint) apikeyHint.style.display = "";
      if (browserHint) browserHint.style.display = "none";
    } else {
      if (apikeyRow) apikeyRow.style.display = "none";
      if (apikeyHint) apikeyHint.style.display = "none";
      if (browserHint) {
        browserHint.style.display = "";
        const supported = _detectBrowserSpeechSupport();
        if (supported) {
          browserHint.className = "voice-browser-hint voice-browser-supported";
          browserHint.textContent = I18n.t("settings.voice.browserSupported");
        } else {
          browserHint.className = "voice-browser-hint voice-browser-unsupported";
          browserHint.textContent = I18n.t("settings.voice.browserUnsupported");
        }
      }
    }
  }
  ...
  function _scheduleVoiceSave() {
    clearTimeout(_voiceSaveTimer);
    _voiceSaveTimer = setTimeout(_saveVoiceSettings, 600);
  }

  function _initVoiceSettings() {
    // Language buttons (settings-lang-btn style)
    const langBtns = document.querySelectorAll("#voice-language-select .settings-lang-btn");
    langBtns.forEach(btn => {
      btn.addEventListener("click", () => {
    ...
    engineBtns.forEach(btn => {
      btn.addEventListener("click", () => {
        engineBtns.forEach(b => b.classList.remove("is-active"));
        btn.classList.add("is-active");
        _updateVoiceEngineUI(btn.dataset.engine);
        _scheduleVoiceSave();
      });
    });
    ...
    const toggleBtn = document.getElementById("voice-apikey-toggle");
    const apikeyInput = document.getElementById("voice-apikey-input");
    ...
    ["voice-apikey-input", "voice-silence-timeout",
     "voice-restart-delay", "voice-exit-words",
     "voice-shortcut-toggle-key", "voice-shortcut-stop-key",
     "voice-shortcut-start-key", "voice-shortcut-voice_mode-key"
    ].forEach(id => {
      const el = document.getElementById(id);
      if (el) el.addEventListener("change", _scheduleVoiceSave);
    });

    // Sound volume slider: live update + auto-save
    const volumeInput = document.getElementById("voice-sound-volume");
    const volumeValue = document.getElementById("voice-sound-volume-value");
  ...
  async function _saveVoiceSettings() {
    const activeEngine = document.querySelector("#voice-engine-select .voice-engine-btn.is-active");
    if (!activeEngine) return;
    const provider = activeEngine.dataset.engine;

    const apikeyInput = document.getElementById("voice-apikey-input");
    const apiKey = apikeyInput ? apikeyInput.value.trim() : "";

    const payload = { asr: { provider } };
    // Only send api_key if it's a real value (not masked, not empty)
    if (apiKey && !apiKey.includes("****")) {
      payload.asr.api_key = apiKey;
    }

    // Language
    const activeLang = document.querySelector("#voice-language-select .settings-lang-btn.active");
    if (activeLang) payload.language = activeLang.dataset.lang;

    // Default mode
    const activeMode = document.querySelector("#voice-mode-select .voice-engine-btn.is-active");
    if (activeMode) payload.default_mode = activeMode.dataset.mode;

    // Behavior
    const silenceInput = document.getElementById("voice-silence-timeout");
    if (silenceInput && silenceInput.value) payload.silence_timeout_ms = parseInt(silenceInput.value, 10);
    const restartInput = document.getElementById("voice-restart-delay");
    if (restartInput && restartInput.value) payload.voice_mode_restart_delay_ms = parseInt(restartInput.value, 10);

    // Shortcuts — only send keys that have a value
    const shortcuts = {};
    ["toggle", "stop", "start", "voice_mode"].forEach(action => {
      const input = document.getElementById("voice-shortcut-" + action + "-key");
      if (input && input.value.trim()) {
        shortcuts[action] = { modifiers: ["Control", "Shift"], key: input.value.trim() };
      }
    });
    if (Object.keys(shortcuts).length) payload.shortcuts = shortcuts;

    // Exit words
    const exitWordsInput = document.getElementById("voice-exit-words");
    if (exitWordsInput) {
      const words = exitWordsInput.value.split(/[,，]/).map(w => w.trim()).filter(Boolean);
      payload.exit_words = words;
    }

    // Sound volume
    const volumeInput = document.getElementById("voice-sound-volume");
    if (volumeInput) payload.sound = { volume: parseFloat(volumeInput.value) };

    const statusEl = document.getElementById("voice-save-status");
    if (statusEl) {
      statusEl.textContent = I18n.t("settings.voice.saving");
      statusEl.className = "voice-save-status";
    }
    ...
    try {
      ...
      if (data.ok) {
        if (statusEl) {
          statusEl.textContent = I18n.t("settings.voice.saved");
          statusEl.className = "voice-save-status voice-save-ok";
        }
        // Reload config in voice.js so the new setting takes effect immediately
        if (typeof Voice !== "undefined" && Voice.reloadConfig) await Voice.reloadConfig();
        // Refresh the masked key display
        _loadVoiceSettings();
      } else {
        if (statusEl) {
          statusEl.textContent = data.error || "Error";
          statusEl.className = "voice-save-status voice-save-error";
        }
      }
    } catch (e) {
      if (statusEl) {
