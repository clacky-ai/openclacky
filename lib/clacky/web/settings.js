...
const Settings = (() => {
  ...
  function open() {
    ...
    _initHomepageSetting();
    if (Clacky.Backup) Clacky.Backup.load();
    _applyAboutTabVisibility();
  }

  let _loadedHomepagePreference = null;

  async function _initHomepageSetting() {
    const section = document.getElementById("settings-homepage-section");
    const select = document.getElementById("settings-homepage-select");
    const saveBtn = document.getElementById("btn-save-default-homepage");
    const restoreBtn = document.getElementById("btn-restore-default-homepage");
    if (!section || !select || !Clacky.ext?.ui?.homepageCandidates) return;

    const candidates = Clacky.ext.ui.homepageCandidates();
    ...
    autoOption.textContent = I18n.t("settings.homepage.auto");
    select.appendChild(autoOption);

    const nativeOption = document.createElement("option");
    nativeOption.value = "native";
    nativeOption.textContent = I18n.t("settings.homepage.native");
    select.appendChild(nativeOption);

    candidates.forEach((candidate) => {
      const option = document.createElement("option");
      option.value = candidate.workspaceId;
      option.textContent = I18n.lang() === "zh"
        ? candidate.titleZh
        : candidate.title;
      select.appendChild(option);
    });

    await Clacky.ext.ui.loadHomepagePreference();
    _loadedHomepagePreference = Clacky.ext.ui.homepagePreference();
    select.value = _loadedHomepagePreference || "auto";
    _syncHomepageButtons();

    if (saveBtn && !saveBtn.dataset.bound) {
      saveBtn.dataset.bound = "1";
      saveBtn.addEventListener("click", _saveHomepagePreference);
      select.addEventListener("change", _syncHomepageButtons);
    }
    if (restoreBtn && !restoreBtn.dataset.bound) {
      restoreBtn.dataset.bound = "1";
      restoreBtn.addEventListener("click", _restoreHomepagePreference);
    }
  }

  function _syncHomepageButtons({ busy = false } = {}) {
    const select = document.getElementById("settings-homepage-select");
    const saveBtn = document.getElementById("btn-save-default-homepage");
    const restoreBtn = document.getElementById("btn-restore-default-homepage");
    const selected = select?.value === "auto" ? null : select?.value;
    if (saveBtn) saveBtn.disabled = busy || selected === _loadedHomepagePreference;
    if (restoreBtn) restoreBtn.disabled = busy || _loadedHomepagePreference == null;
  }

  async function _saveHomepagePreference() {
    const select = document.getElementById("settings-homepage-select");
    const status = document.getElementById("settings-homepage-status");
    const value = select.value === "auto" ? null : select.value;
    _syncHomepageButtons({ busy: true });
    try {
      const saved = await Clacky.ext.ui.selectHomepage(value);
      if (!saved) throw new Error(I18n.t("settings.homepage.failed"));
      _loadedHomepagePreference = value;
      status.textContent = I18n.t("settings.homepage.saved");
      status.className = "model-test-result result-ok";
    } catch (error) {
      status.textContent = error.message || I18n.t("settings.homepage.failed");
  ...
  async function _restoreHomepagePreference() {
    const select = document.getElementById("settings-homepage-select");
    const status = document.getElementById("settings-homepage-status");
    _syncHomepageButtons({ busy: true });
    try {
      await Clacky.ext.ui.selectHomepage(null);
      _loadedHomepagePreference = null;
      select.value = "auto";
      status.textContent = I18n.t("settings.homepage.restored");
      status.className = "model-test-result result-ok";
    } catch (error) {
      status.textContent = error.message || I18n.t("settings.homepage.failed");
      status.className = "model-test-result result-fail";
