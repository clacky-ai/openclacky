...
const Settings = (() => {
  ...
  function _openModalDuplicate(index) {
    ...
    // Copy API format override from source model
    document.getElementById("model-modal-api-format").value = source.api_format || "";
    _syncRemarkField();
  }

  // Always visible: the backend honors an explicit per-model api_format for
  // every provider (user_override wins over preset resolution in providers.rb),
  // and "Auto" simply means preset + per-model resolution.
  ...
  async function _saveModalModel() {
    ...
    // An explicit choice is honored for every provider: user_override takes
    // priority over preset + per-model resolution (providers.rb).
    const api_format = document.getElementById("model-modal-api-format").value.trim() || null;
