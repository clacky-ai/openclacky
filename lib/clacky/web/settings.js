...
const Settings = (() => {
  ...
  // custom (non-preset) providers: they are shown once the dropdown actually
  ...
  // capability table server-side, so no per-model override is needed there.
  function _syncRemarkField() {
    ...
    if (noVision) noVision.style.display = isCustom ? "" : "none";
  }
