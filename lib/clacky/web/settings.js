...
const Settings = (() => {
  ...
  // Model list pulled from an endpoint for the modal's custom-provider flow:
  // `{ key, models }` where key is base_url+api_key, so editing either field
  // silently discards a list fetched for the previous pair. Filled by the
  // automatic fetch triggered on API-key blur / Base-URL blur / dropdown open.
  let _modalRemoteModels = null;
  // Guards against overlapping fetches (blur → open-dropdown can fire within
  // the same second).
  let _remoteLoadInFlight = false;
  ...
  function _openModal(index = -1) {
    ...
    // Drop any model list fetched for a previously opened model/endpoint.
    _modalRemoteModels = null;
  ...
  function _initModal() {
    ...
    function _openModelDropdown() {
      ...
      // Opening the dropdown is an explicit "show me what this endpoint
      // offers" intent — fetch when there is no fresh list yet. Masked keys
      // are allowed on this trigger only: the server resolves them from the
      // stored config, so editing a saved custom model can still list it.
      _loadRemoteModels({ allowMasked: true });
    }
    ...
    // Update model dropdown when base_url changes, and refresh the fetched
    // list for the new endpoint (skipped for presets/masked keys — see
    // _shouldFetchRemoteModels).
    baseUrlInput.addEventListener("blur", () => {
      _updateModalModelDropdown();
      _loadRemoteModels();
    });

    // Finishing the API key is the moment a custom endpoint becomes listable:
    // fetch its models in the background so the dropdown is already populated
  ...
  function _updateModalModelDropdown() {
    ...
    const picked = _modalDropdownModels(provider);
    ...
    const status = picked.remote
      ? `<div class="model-dropdown-status">${I18n.t("settings.models.fetchedFrom", { endpoint: _esc(picked.endpoint), count: picked.models.length })}</div>`
      : "";

    modelDropdown.innerHTML = status;
    picked.models.forEach(m => {
      const opt = document.createElement("div");
      opt.className = "model-dropdown-option";
      opt.dataset.value = m;
      opt.appendChild(ModelPicker.vendorBadge(m));
      const label = document.createElement("span");
      label.className = "model-dropdown-option-label";
      label.textContent = m;
      opt.appendChild(label);

      // Price ratio and the "auto" hint come from the shared ModelPicker
      // logic, so this list matches the in-session model lists.
      const right = document.createElement("span");
      right.className = "model-dropdown-option-right";
      opt.appendChild(right);
      ModelPicker.decorateModelRow(opt, m, right);
  ...
  // Which model ids the dropdown should offer: the list fetched from the
  // endpoint the user typed wins, but only while that exact base_url+api_key
  // pair is still in the form. Otherwise fall back to the matched provider's
  // preset models.
  function _modalDropdownModels(provider) {
    ...
    const fresh = _modalRemoteModels && _modalRemoteModels.key === _remoteModelsKey(baseUrl, apiKey);
    return fresh
      ? { models: _modalRemoteModels.models, remote: true, endpoint: _prettyEndpoint(baseUrl) }
      : { models: provider?.models || [], remote: false, endpoint: "" };
  }

  // Cache identity for a fetched list — editing Base URL or API Key should not
  // keep offering models that were listed for the previous pair.
  function _remoteModelsKey(baseUrl, apiKey) {
    return baseUrl + "\u0000" + apiKey;
  }

  // "https://llm.example.com/v1/" → "llm.example.com/v1" for the status row.
  function _prettyEndpoint(rawUrl) {
  ...
  function _modalApiFormat() {
    return _modalProviderIsCustom ? ((_apiFormatSelect && _apiFormatSelect.value) || null) : null;
  }

  // Decide whether a trigger (API-key blur, Base-URL blur, dropdown open)
  // should fetch the endpoint's model list. Shared by all triggers so they
  // cannot double-fire or race each other.
  //
  // `allowMasked` is true only for the dropdown-open trigger: there the user
  // explicitly asked for a list, and the server resolves a masked key back to
  // the stored secret via the model id/index. Blur-triggered fetches skip
  // masked keys (upstream guidance) so merely opening a model for editing
  // never fires a request at the provider.
  function _shouldFetchRemoteModels({ allowMasked }) {
    if (_remoteLoadInFlight) return false;
    const baseUrl = (document.getElementById("model-modal-baseurl")?.value || "").trim();
    const apiKey = (document.getElementById("model-modal-apikey")?.value || "").trim();
    if (!baseUrl || !apiKey) return false;
    if (!allowMasked && apiKey.includes("****")) return false;
    // Preset providers keep their static model list (upstream guidance).
    if (_getModalCurrentProvider()) return false;
    // Same pair already fetched — reuse the cache.
    if (_modalRemoteModels && _modalRemoteModels.key === _remoteModelsKey(baseUrl, apiKey)) {
      return false;
    }
    return true;
  }

  // Query the typed endpoint for the model ids it advertises and offer them in
  // the Model dropdown. Triggered automatically, never by a dedicated button.
  async function _loadRemoteModels({ allowMasked = false } = {}) {
    if (!_shouldFetchRemoteModels({ allowMasked })) return;

    const dropdown = document.getElementById("model-modal-model-dropdown");
    const baseUrl = (document.getElementById("model-modal-baseurl")?.value || "").trim();
    const apiKey = (document.getElementById("model-modal-apikey")?.value || "").trim();
    const index = parseInt(document.getElementById("model-modal-index").value, 10);
    const sourceId = document.getElementById("model-modal-source-id").value || null;
    ...
    // Paint progress/errors only while the dropdown is open, checked live at
    // paint time: a blur-triggered fetch runs in the background, and the user
    // may open the dropdown while it is still in flight — the outcome (error,
    // empty, loading) is exactly what they are waiting for then. Failures and
    // empty results are deliberately not cached, so reopening retries.
    function _paint(text) {
      if (dropdown.style.display !== "block") return;
      dropdown.innerHTML = `<div class="model-dropdown-empty">${text}</div>`;
    }

    _remoteLoadInFlight = true;
    let result;
    try {
      _paint(I18n.t("settings.models.loadingModels") || "Loading models…");
      result = await ModelTester.listModels({
        base_url: baseUrl,
        api_key: apiKey,
        api_format: _modalApiFormat(),
    ...
    if (!result.ok) {
      _paint(I18n.t("settings.models.loadFailed", { msg: _esc(result.message || "") })
        || "Couldn't load models");
      return;
    }

    if (result.models.length === 0) {
      _paint(I18n.t("settings.models.loadEmpty") || "Endpoint returned no models");
      return;
    }

    _modalRemoteModels = { key: _remoteModelsKey(baseUrl, apiKey), models: result.models };
    _updateModalModelDropdown();
  }
