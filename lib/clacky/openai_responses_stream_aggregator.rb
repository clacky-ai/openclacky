...
module Clacky
  ...
  # with a "response.completed" (or "response.incomplete") event.
  class OpenAIResponsesStreamAggregator
    ...
    def handle(data_str)
      ...
      when "response.completed"
        ...
        if response && response["output"]
          sync_from_final_output(response["output"])
        end
    ...
    private def sync_from_final_output(output)
      ...
      # Only sync if we have no accumulated text and no function calls
      # (i.e. we missed all the incremental events).
      return unless @text.empty? && @function_calls.empty?
