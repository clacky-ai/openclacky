...
module Clacky
  class Client
    ...
    def send_messages_with_tools(messages, model:, tools:, max_tokens:, enable_caching: false, reasoning_effort: nil, on_chunk: nil, agent_role: nil, agent_iteration: nil, agent_retries: nil, agent_upstream_fails: nil, agent_upgrade_fails: nil)
      api_model = Providers.resolve_api_model(base_url: @base_url, api_key: @api_key, model: model)
      ...
      response =
        if bedrock?
          ...
          streaming_used = !on_chunk.nil?
          send_openai_request(cloned, api_model, tools, max_tokens, caching_enabled, reasoning_effort: reasoning_effort, on_chunk: wrapped_on_chunk, capability_model: model, agent_role: agent_role, agent_iteration: agent_iteration, agent_retries: agent_retries, agent_upstream_fails: agent_upstream_fails, agent_upgrade_fails: agent_upgrade_fails)
        end
    ...
    def send_openai_request(messages, model, tools, max_tokens, caching_enabled, reasoning_effort: nil, on_chunk: nil, capability_model: nil, agent_role: nil, agent_iteration: nil, agent_retries: nil, agent_upstream_fails: nil, agent_upgrade_fails: nil)
      ...
      return send_openai_stream_request(body, on_chunk, agent_role: agent_role, agent_iteration: agent_iteration, agent_retries: agent_retries, agent_upstream_fails: agent_upstream_fails, agent_upgrade_fails: agent_upgrade_fails) if on_chunk

      response = openai_connection.post("chat/completions") do |r|
        r.body = body.to_json
        set_agent_headers(r, agent_role, agent_iteration, agent_retries, agent_upstream_fails, agent_upgrade_fails)
      end
    ...
    # task is actually going (which iteration, how many consecutive misses).
    private def set_agent_headers(req, role, iteration, retries, upstream_fails, upgrade_fails)
      req.headers["X-Clacky-Agent-Role"] = role if role
      req.headers["X-Clacky-Agent-Iteration"] = iteration.to_s if iteration
    ...
    private def send_openai_stream_request(body, on_chunk, agent_role: nil, agent_iteration: nil, agent_retries: nil, agent_upstream_fails: nil, agent_upgrade_fails: nil)
      ...
      response = openai_connection.post("chat/completions") do |req|
        req.body = stream_body.to_json
        set_agent_headers(req, agent_role, agent_iteration, agent_retries, agent_upstream_fails, agent_upgrade_fails)
        req.options.on_data = proc do |chunk, _bytes_received, _env|
          Clacky::Shutdown.checkpoint!
          sse_buf << chunk
