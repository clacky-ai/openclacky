        ...
        class WSClient
          ...
          def connect_and_listen
            ...
            @ws.on :error do |event|
              warn "WebSocket error: #{event.message}"
            end
            ...
            warn "[feishu-ws] connection lost, reconnecting..."
          ensure
          ...
          def fetch_ws_endpoint
            ...
            raise "Failed to get WebSocket endpoint: #{data['msg']}" if data["code"] != 0
          ...
          def handle_frame(raw)
            ...
            warn "Failed to handle WebSocket frame: #{e.message}"
          end
          ...
          def handle_control_frame(frame, msg_type)
            ...
            when "handshake"
              ...
              if status != "200"
                warn "Feishu WS handshake failed: #{frame[:headers]['handshake-msg']}"
              end
            end
          end
