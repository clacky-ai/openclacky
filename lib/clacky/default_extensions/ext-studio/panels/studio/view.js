...
(function () {
  const STUDIO_I18N = {
    en: {
      ...
      "extlist.badge.unlisted": "Unlisted",
      "extlist.unlisted.hint": "Taken down by the marketplace. Publishing a new version restores it.",
    ...
    zh: {
      ...
      "extlist.badge.unlisted": "已下架",
      "extlist.unlisted.hint": "已被市场下架，重新发布新版本可恢复上架。",
    ...
    async function loadPublished() {
      ...
      try {
        ...
        exts.forEach((e) => {
          ...
          const head = el("div", { class: "studio-skill-head" });
          const unlisted = listingState(e) === "unlisted";
          head.appendChild(el("span", { class: "studio-skill-name", text: e.name || e.id }));
          const stateBadge = el("span", { class: `studio-skill-badge studio-skill-badge-${listingBadgeKind(e)}`, text: listingBadgeText(e) });
          if (unlisted) stateBadge.setAttribute("data-tooltip", t("extlist.unlisted.hint"));
          ...
          // A taken-down extension has nothing left to unpublish — the only way
          // back on the marketplace is shipping a new version.
          if ((!brandOwner || e.origin === "self") && !unlisted) {
  ...
  // An admin takedown keeps `status: "published"` on the platform record —
  // only hub_status flips (hidden/removed/quarantined). Map both fields to one
  // badge state so a taken-down extension never reads as live.
  function listingState(ext) {
    if (!ext) return "published";
    if (ext.status === "draft") return "draft";
    const hub = ext.hub_status;
    if (hub && hub !== "active") return "unlisted";
    return "published";
  }

  function listingBadgeKind(ext) {
    const state = listingState(ext);
    if (state === "draft") return "local";
    return state === "unlisted" ? "unlisted" : "published";
  }

  function listingBadgeText(ext) {
    const state = listingState(ext);
    if (state === "draft") return t("extlist.badge.draft");
    return state === "unlisted" ? t("extlist.badge.unlisted") : t("extlist.badge.published");
  }
  ...
  function createExtensionsPanel() {
    ...
    function badge(text, kind, tooltip) {
      const node = el("span", { class: "studio-skill-badge studio-skill-badge-" + kind, text });
    ...
    function cloudCard(ext) {
      const card = el("div", { class: "studio-skill-card" });
      const head = el("div", { class: "studio-skill-head" });
      const unlisted = listingState(ext) === "unlisted";
    ...
    function localCard(ext) {
      ...
      const head = el("div", { class: "studio-skill-head" });
      head.appendChild(el("span", { class: "studio-skill-name", text: `${ext.name} (${ext.id})` }));
      const publishedEntry = brandOwner ? brandEntry : marketEntry;
      head.appendChild(published ? badge(listingBadgeText(publishedEntry), listingBadgeKind(publishedEntry)) : badge(t("extlist.badge.local"), "local"));
      if (ext.disabled) {
  ...
  style.textContent = `
    ...
    .studio-skill-badge-unlisted { background: var(--color-warning-bg, var(--color-bg-hover)); color: var(--color-warning, var(--color-text-secondary)); }
